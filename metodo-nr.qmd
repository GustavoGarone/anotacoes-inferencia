...
```{Python}
# Código em Python (TODO/TESTE)
```

#### R

```{R}
# Código em R (TODO/TESTE)
```
...
function main()
  ...
  while erro > erromax
    itsepoca = 0
    for i in 1:length(x)
      append!(theta, theta[epoca] + delta *(log(x[i]) -  digamma(theta[i])))
      erro = abs(theta[epoca+1] - theta[epoca])
      itsepoca = i
    end
    totalit += itsepoca
    if epoca % 10 == 0
      println("Erro na epoca $(epoca-1) e iteração $totalit: $erro")
      println("theta estimado na epoca $(epoca-1): $(theta[totalit])")
    end
    epoca += 1
  end
  println("Theta final: $(theta[length(theta)])")
