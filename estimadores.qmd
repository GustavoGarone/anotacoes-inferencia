...
São estatísticas cujo objetivo é estimar uma [quantidade de
interesse](quantidade-de-interesse.qmd).
...
Portanto, *estimativas são valores numéricos*
...
$$
\begin{aligned}
E_\theta(T_1(X_1,X_2)) &= E_\theta(X_1) \\
&= 0 \cdot (1-\theta)^2 + \theta(1-\theta) + 0 \cdot \theta(1-\theta) + \theta^2 \\
&= \theta. \\[0.5em]
E_\theta(T_2(X_1,X_2)) &= \theta. \\[0.5em]
E_\theta(T_3(X_1,X_2)) &= E_\theta(X_1+X_2) = 2\theta. \\[0.5em]
E_\theta(T_4(X_1,X_2)) &= E_\theta(\max\{X_1,X_2\}) \\
&= 2\theta(1-\theta) + \theta^2 = 2\theta-\theta^2. \\[0.5em]
E_\theta(T_5(X_1,X_2)) &= E_\theta(\min\{X_1,X_2\}) = \theta^2. \\[0.5em]
E_\theta(T_6(X_1,X_2)) &= \frac{1}{2}\theta(1-\theta).
\end{aligned}
$$
...
### Estimadores não-viciados ou não-enviesados {#sec-est-naovies}
