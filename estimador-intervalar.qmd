...
$$
P_{\theta} (I_{1}(\pmb{X}_{n}) \leq \theta \leq I_{2} (\pmb{X}_{n})) = \gamma
...
$$
P_{\theta} (I_{1} (\pmb{X}_{n}) \leq \theta \leq I_{2} (\pmb{X}_{n})) \geq
...
podemos afirmar que a probabilidade de esse IC observado conter o valor de $\theta$
...
3. $\sum\limits^{n}_{i=1}\frac{(X_{i} - \bar{X})^2}{\sigma^2} \sim \chi^2_{n-1}$
...
{\sqrt{\frac{\sum_{i=1}^n (X_i - \bar{X})^2}{\sigma^2} \cdot \frac{1}{n-1}}}$
...
Note que $$\sum\limits^{n}_{i=1}\frac{(X_{i} - \bar{X})^2}{\sigma^2} \sim \chi^2_{n-1}$$
...
:::{.callout-note title="Média conhecida"}
Neste exercício, a média populacional é conhecida e igual a zero. Portanto, também poderíamos usar a quantidade pivotal
$$
\frac{\sum_{i=1}^n X_i^2}{\theta} \sim \chi^2_n,
$$
obtendo um intervalo de confiança com $n$ graus de liberdade.

Entretanto, aplicamos aqui a construção geral anterior, baseada em $S^2$ e com $n-1$ graus de liberdade, pois ela continua
sendo exata quando a média é conhecida e será útil também quando ela for desconhecida. A alternativa com média conhecida é
retomada na discussão de [quantidades pivotais](intervalos-confianca.qmd).
:::
...
Isso é importante para diferenciar a interpretação frequentista (Theta do
espaço paramétrico) da Bayesiana
(Theta como variável aleatória). Dessa forma, estaria ***incorreto*** na
estatística clássica dizer que
