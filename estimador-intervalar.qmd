...
**Observação**: $I_{1} (\pmb{X}_{n})$ e $I_{2} (\pmb{X}_{n})$ são [estatísticas](estatisticas.qmd) e são tais que
...
Em que $s^2 (\pmb{x}_{n}) = \frac{1}{n-1}\sum^{k}_{i=1}(X_i - \bar{x})^2$ e $t_{\gamma,(n-1)}$ deve ser calculado da tabela
...
4. $\sqrt{n} \frac{(\bar{X} - \mu)}{\sqrt{s^2}} \sim t_{(n-1)}$
  1. $\sqrt{n}\frac{\bar{X} - \mu}{\sqrt{s^2}} = \sqrt{n}\frac{\bar{X} - \mu}{\sqrt{s^2}} \cdot
\frac{\sqrt{\sigma^2}}{\sqrt{\sigma^2}}  = \frac{\sqrt{n}\frac{\bar{X} - \mu}{\sqrt{\sigma^2}}}
{\sqrt{\frac{\sum(X_i - \bar{X})^2}{\sigma^2} \cdot \frac{1}{n-1}}}$
  2. Se $Z \sim N(0,1)$ e $W \sim \chi^{2}_{k}$, então $t = \frac{Z}{\sqrt{\frac{W}{k}}} \sim t_{k}$
