...
Temos nossa tabela de frequências esperadas [sob](teste-hipotese.qmd) $H_{0}$ (Independência)
$$
\begin{array}{c|cccc|c}
X\setminus Y  & A_{1} & A_{2} & \dots & A_{k} & \mathrm{Total}~X \\
\hline
...
$$
\chi^2_{\mathrm{obs}} = \sum^k_{i=1}\sum^l_{j=1} \frac{(O_{ij}-E_{ij})^2}{E_{ij}}
$$
Sob $H_{0}$, ou seja,
$$
\chi^2_{\mathrm{obs}}\sim \chi^2_{(k-1)(l-1)}
$$
Dessa forma, rejeitamos a hipótese $H_{0}$ a $\alpha$ graus de liberdade se
$$
\chi^2_{\mathrm{obs}} > c_{p}
$$
