...
No caso em que $X,Y$ são dependentes, as amostras são $(\pmb{X}_{n})$ [amostras aleatórias](populacao-e-amostra.qmd#sec-aa)
de $X$ e $(\pmb{Y}_{n})$ a.a de $Y$ tais que $X_{i},Y_{i}$ são dependentes.
...
\end{cases} $$
Considere $(\pmb{X}_{n})$ amostra aleatória de $X$ e $(\pmb{Y}_{m})$ amostra aleatória de $Y$. Estamos interessados em testar as hipóteses:
$$
1.
\begin{cases}
H_{0}:\mu_{X}\geq \mu_{Y} \\
