---
date: 2024-10-01T14:04:00
tags:
  - Inferência
  - Anotações
---

Seja $(X_1,\ldots, X_n)$ [[População e amostra#Amostra Aleatória|a.a.]] de $X\sim f_\theta, \theta \in \Theta \subseteq \mathbb{R}$.

Um intervalo de confiança com coeficiente de confiança $\gamma$ é um intervalo aleatório que satisfaz:
...
**Observação**: $I_{1} (\pmb{X}_{n})$ e $I_{2} (\pmb{X}_{n})$ são [[Estatísticas e Estimadores#Estatísticas|estatísticas]] e são tais que $I_{1} (\pmb{X}_{n}) \leq I_{2} (\pmb{X}_{n})$
...
$$
- c_{\gamma} \leq Z \leq c_{\gamma} \Leftrightarrow -c_{\gamma} \leq \frac{\sqrt{n}(\bar{X}-\theta)}{\sqrt{\sigma^{2}}} \leq c_{\gamma} \Leftrightarrow \bar{X} - c_{\gamma} \frac{\sqrt{\sigma^2}}{\sqrt{n}} \leq \theta \leq \bar{X} + c_{\gamma} \frac{\sqrt{\sigma^2}}{\sqrt{n}}
$$
Portanto,
$$
P_{\theta}\left(\bar{X} - c_{\gamma} \frac{\sqrt{\sigma^2}}{\sqrt{n}} \leq \theta \leq \bar{X} + c_{\gamma} \frac{\sqrt{\sigma^2}}{\sqrt{n}}\right) = \gamma \forall \theta \in \Theta
$$

Dessa forma,

$\bar{X} - c_{\gamma} \frac{\sqrt{\sigma^2}}{\sqrt{n}} \leq \theta \leq \bar{X} + c_{\gamma} \frac{\sqrt{\sigma^2}}{\sqrt{n}}$ é um intervalo de confiança cujo coeficiente de confiança é $\gamma$. 
...
4-) $\sqrt{n}\frac{\bar{X} - \mu}{\sqrt{s^2}} = \sqrt{n}\frac{\bar{X} - \mu}{\sqrt{s^2}} \cdot \frac{\sqrt{\sigma^2}}{\sqrt{\sigma^2}}  = \frac{\sqrt{n}\frac{\bar{X} - \mu}{\sqrt{\sigma^2}}}{\sqrt{\frac{\sum(X_i - \bar{X})^2}{\sigma^2} \cdot \frac{1}{n-1}}}$
...
$$
\begin{aligned}
\mathrm{IC} (\sigma^2, \gamma) = \left [\frac{(n-1)s^2(\pmb{X}_{n})}{q^{(2)}_{\gamma, n-1}},\frac{(n-1)s^2(\pmb{X}_{n})}{q^{(1)}_{\gamma, n-1}}\right] \\
\mathrm{IC}_{\mathrm{Obs}} (\sigma^2, \gamma) = \left [\frac{(n-1)s^2(\pmb{x}_{n})}{q^{(2)}_{\gamma, n-1}},\frac{(n-1)s^2(\pmb{x}_{n})}{q^{(1)}_{\gamma, n-1}}\right]
\end{aligned}
$$
em que $s^2(\pmb{X}_{n}) = \frac{1}{n-1} \sum^{n}_{i=1} (X_i - \bar{X})^2$ e $s^2(\pmb{x}_{n}) = \frac{1}{n-1} \sum^{n}_{i=1} (x_i - \bar{x})^2$
E $q^{(1)}_{\gamma, n-1}, q^{(2)}_{\gamma, n-1}$ são obtidos calculando $P(q^{(1)}_{\gamma, n-1} \leq W \leq q^{(2)}_{\gamma, n-1}) = \gamma$ no qual $W\sim \chi^{2}_{n-1}$ e $P(\chi^2_{n-1} \leq q^{(1)}_{\gamma, n-1}) = \frac{1-\gamma}{2} = P( \chi^2_{n-1} \geq q^{(2)}_{\gamma,n-1})$
...
Note que
$W \sim \chi^{2}_{n-1}$, ou seja, $P(q^{(1)}_{\gamma, n-1} \leq W \leq q^{(2)}_{\gamma, n-1}) = \gamma$ . Assim, $W = \frac{(n-1)s^2}{\sigma^2}$.
...
Construa um intervalo de confiança para a variância (populacional) $\theta$
com coeficiente de confiança $\gamma = 95\%$.
...
e $q^{1}_{\gamma, n-1}, q^{2}_{\gamma, n-1}$ satisfazem as fronteiras que delimitam uma área de $\gamma$ em torno da média. Nesse caso, como $n = 10$, $q^{1}_{9} = 2.7, q^{2}_{9} = 19.023$. Dessa forma, $\bar{x} = 0.28, \sum_{i}x_{i}^2=2.308 \Rightarrow s^2(\pmb{x}_{n}) = \frac{10}{9}(2.308 - 0.28^2) = 2.48$
...
Sabemos que, pelo [[Teorema do Limite Central]]
$$
...
$$
Observe que, se $Z \sim N(0,1)$ então $-c_{\gamma}\leq Z \leq c_{\gamma} \Leftrightarrow -c_{\gamma } \leq \frac{\sqrt{ n }(\bar{X}-\theta)}{\sqrt{ \bar{X}(1-\bar{X}) }}\leq c_{\gamma}$
$$
\Leftrightarrow \bar{X} - c_{\gamma} \sqrt{ \frac{\bar{X} (1-\bar{X})}{n} } \leq \theta \leq \bar{X} + c_{\gamma} \sqrt{ \frac{\bar{X} (1-\bar{X})}{n} }
$$
Então, seja $c_{\gamma}$ tal que
$$
\begin{aligned}
&P (-c_{\gamma}\leq N(0,1)\leq c_{\gamma }) = \gamma \\
\Rightarrow &P_{\theta}\left(\bar{X} - c_{\gamma} \sqrt{ \frac{\bar{X} (1-\bar{X})}{n} } \leq \theta \leq \bar{X} + c_{\gamma} \sqrt{ \frac{\bar{X} (1-\bar{X})}{n} }\right) \approx \gamma~ \forall \theta \in \Theta = [0,1]
\end{aligned}
$$
...
$$
IC(\theta, \gamma) = \left[\bar{X} - c_{\gamma} \sqrt{ \frac{\bar{X} (1-\bar{X})}{n} }, \bar{X} + c_{\gamma} \sqrt{ \frac{\bar{X} (1-\bar{X})}{n} }\right] \cap \Theta
$$
...
$$
\begin{aligned}
\mathrm{IC}_{\mathrm{Obs}}(\theta, 99\%) &= \left[0.25- 2.58 \frac{\sqrt{ 3 }}{4 \sqrt{  8 }}, 0.25 - 2.58 \frac{\sqrt{ 3 }}{4 \sqrt{  8 }}\right] \cap [0,1] \\
...
Por sua vez, um IC otimista é calculado usando o valor de $\theta$ obtido através do [[Função de Verossimilhança e Distribuição Amostral#Estimação via máxima verossimilhança (EMV)|EMV]] para $\theta$, no caso Bernoulli, usaríamos essa variância como $\bar{X}(1-\bar{X})$
