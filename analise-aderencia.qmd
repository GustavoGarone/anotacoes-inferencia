...
Neste teste comparamos a frequência observada com a frequência esperada em $k$ eventos disjuntos e distintos observáveis.
$$
\begin{array}{c|cccc}
...
Em que observou-se uma [amostra](populacao-e-amostra.qmd#sec-ao) de tamanho $n$. Temos também que $E_{i}$ é o valor esperado
do número de eventos $i$ sob $H_{0}$
$$
\mathrm{Freq. Esperada} = E_{i} = P_{0i} \cdot n
$$

e $\mathrm{Freq. Observada} = O_{i}$ é o número real de eventos $i$ observados na amostra.
...
## Exemplo
Considere que queremos verificar se os números sorteados nos concursos da Mega
Sena são de fato uniformemente distribuídos.
...
```{julia}
#| label: fig-analise-aderencia-qui-quadrado-julia
#| fig-cap: 'Distribuição $\chi^2_{59}$ e ponto crítico do teste de aderência'
# Anal. Aderência
# H0 = P=P0
#
# Mega Sena. Observar apenas o primeiro número de cada sorteio

Random.seed!(1)
amostra = sample(1:60, 2800)
O = counts(amostra, 1:60)
E = 2800 / 60
...
```{python}
#| label: fig-analise-aderencia-qui-quadrado-python
#| fig-cap: 'Distribuição $\chi^2_{59}$ e ponto crítico do teste de aderência'
import matplotlib
matplotlib.use("Agg")
...
# H0: P = P0
#
# Mega-Sena: observamos apenas o primeiro número de cada sorteio.
rng = np.random.default_rng(1)
amostra = rng.choice(np.arange(1, 61), size=2_800)
O = np.bincount(amostra, minlength=61)[1:]
E = 2_800 / 60

qui_quadrado = np.sum((E - O) ** 2 / E)
quantil = chi2.ppf(0.95, df=59)

t = np.linspace(0, 120, 500)
fig, ax = plt.subplots()
ax.plot(t, chi2.pdf(t, df=59), color="steelblue")
ax.axvline(
    quantil,
    color="firebrick",
    label=fr"$c = {quantil:.2f}$",
)
ax.set_xlim(0, 120)
ax.legend()
plt.show()
...
```{r}
#| label: fig-analise-aderencia-qui-quadrado-r
#| fig-cap: 'Distribuição $\chi^2_{59}$ e ponto crítico do teste de aderência'
# Análise de aderência
# H0: P = P0
#
# Mega-Sena: observamos apenas o primeiro número de cada sorteio.
set.seed(1)
amostra <- sample(1:60, size = 2800, replace = TRUE)
O <- tabulate(amostra, nbins = 60)
E <- 2800 / 60

qui_quadrado <- sum((E - O)^2 / E)
quantil <- qchisq(0.95, df = 59)
...
abline(v = quantil, col = "firebrick")
legend(
  "topright",
  legend = sprintf("c = %.2f", quantil),
  col = "firebrick",
  lty = 1,
  bty = "n"
)
...
Logo, como $\chi^2=63.8 < 77.93$, concluímos que, sob $H_{0}$, não há evidências de que o modelo não seja equiprovável
a $5\%$ de significância de estatística.

## K-Grupos
O exemplo a seguir é o Exemplo 14.6 de
@morettinEstatisticaBasica10a2024 [p. 427].
...
dessa distribuição normal. (Primeiro padronizamos, encontramos os valores pela
