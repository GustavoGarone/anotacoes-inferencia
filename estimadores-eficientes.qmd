...
Logo, $T(\boldsymbol{X}_n)$ é assintóticamente eficiente para $g(\theta)$
...
\iff n\theta(\mathrm{e}^{-\frac{1}{n}} - 1) &= -\theta + \underbracket{\left(\frac{1}{n} + \frac{\theta}{2n}+ \dots\right)}_{h(\theta,n)} \\
...
Apesar de $T_{MV}(\boldsymbol{X}_n)$ ser viciado para $g(\theta) = \mathrm{e}^{\theta}$, ela é não-viciada assintóticamente.
...
$$
e concluímos que $T_{MV}(\boldsymbol{X}_n) = \mathrm{e}^{-\bar{X}}$ é assintóticamente eficiente.
