...
em que $C(\dot)$ e $d(\dot)$ são funções de "$\theta$" cujas formas são conhecidas e $T(\dot)$ e $S(\dot)$ são funções
...
Note que $\mathfrak{X}=\{x:f_\theta(x)>0\} = (0, \infty)$ não depende de "$\theta$". Além disso, como $\theta > 0$,
...
#### Exemplo 6 - Contraexemplo uniforme
...
#### Exemplo 7 - Contraexemplo Normal (Média e Variância)
...
portanto, não é possível definir $C(\theta),T(x),d(\theta)$ e $S(x)$ tais que $C(\dot), T(\dot)$ representem
...
## Família Exponencial k-dimensional
...
2. As funções $C_1(\dot),\dots,C_k(\dot)$ e $d(\dot)$ dependem apenas de "$\theta$" (formas conhecidas) e
3. As funções $T_1(\dot),\dots,T_l(\dot)$ e $S(\dot)$ dependem apenas de $x$ (formas conhecidas)
...
$$
\begin{array}{cc}
C_1(\theta) = -\frac{1}{2\theta}, & T_1(x) = x^2 \\
