...
plt.title(r'$\alpha = \pi (\theta) = (1-\theta)^{2}$')
...
ggplot() +
  ...
  labs(
    ...
    title = expression(alpha == pi(theta) ~ "=" ~ (1 - theta)^2)
  ) +
  theme_minimal()
```
:::
...
plot(
    ...
    ylabel = L"1 - \pi (\theta)",
    xlabel = L"\theta",
    title = L"\beta  = 1 - \pi (\theta) = 1-(1-\theta)^{2}"
)
```
:::
...
a = plot(
    poder, xlims = (mu0 - 6 * sqrt(sigma2 / n), mu0 + 6 * sqrt(sigma2 / n)), ylims = (0, 1.001), label = "", title = L"\alpha = \pi (\theta)",
...
beta(mu0)
b = plot(
    beta, xlims = (mu0 - 6 * sqrt(sigma2 / n), mu0 + 6 * sqrt(sigma2 / n)), ylims = (-0.01, 1.001), label = "", title = L"\beta",
