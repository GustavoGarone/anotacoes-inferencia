...
Note que $\bar{X}$ é um [estimador](estimadores.qmd) para $\theta$ e $\bar{x}$ é uma [estimativa](estimadores.qmd#sec-estimativa).
...
Sabemos que $\sum_{i=1}^{n} X_{i}\sim \mathrm{Bin}(n, \theta)$, logo
$$
\begin{aligned}
...
Para $n=10$,
$$
\beta(\theta) = P_{\theta}\left( 3\leq \sum_{i=1}^{10} X_{i} \leq 7 \right) = P\left( 3\leq \mathrm{Bin}(10,\theta)
...
### Julia
```{julia}
#| label: fig-teste-hipotese-alfa-julia
#| fig-cap: 'Função poder $\alpha=\pi(\theta)=(1-\theta)^2$'
using Plots, Distributions, LaTeXStrings

function poder(theta)
    dist = Binomial(2, theta)
		# Calcula a densidade acumulada até o ponto especificado
...
### Python
```{python}
#| label: fig-teste-hipotese-alfa-python
#| fig-cap: 'Função poder $\alpha=\pi(\theta)=(1-\theta)^2$'
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
def poder(theta):
    return binom.cdf(2 * 0.4, 2, theta)

# Precisamos gerar os valores de theta (como o xlims)
# Verifique a documentação para entender melhor!
val_theta = np.linspace(0.6, 1, 1_000)
# Criamos os valores de y para serem plotados
val_y = [poder(t) for t in val_theta]

plt.figure(figsize=(8, 5))
plt.plot(val_theta, val_y, color='blue')
...
### R
```{r}
#| label: fig-teste-hipotese-alfa-r
#| fig-cap: 'Função poder $\alpha=\pi(\theta)=(1-\theta)^2$'
# Biblioteca de gráficos moderna, parte do Tidyverse
library(ggplot2)

# Função poder vetorizada
poder <- function(theta) pbinom(0.8, 2, theta)

ggplot() +
  xlim(0.6, 1) +
  geom_function(fun = poder, color = "blue") +
  labs(
    x = expression(theta),
...
### Julia
```{julia}
#| label: fig-teste-hipotese-beta-julia
#| fig-cap: 'Função erro tipo II $\beta=1-\pi(\theta)=1-(1-\theta)^2$'
using Plots, Distributions, LaTeXStrings
function poder(theta)
    dist = Binomial(2, theta)
...
plot(
    poder,
    xlims = (0, 0.601),  #.601 para exibir o último xtick
    legend = false,
    ylabel = L"1 - \pi (\theta)",
    xlabel = L"\theta"
)
...
### Python
```{python}
#| label: fig-teste-hipotese-beta-python
#| fig-cap: 'Função erro tipo II $\beta=1-\pi(\theta)=1-(1-\theta)^2$'
import numpy as np
import matplotlib.pyplot as plt
from scipy.stats import binom
...
def poder(theta):
    # P(X > 0,8) = P(X >= 1), para X com distribuição Binomial(2, theta)
    return binom.sf(2 * 0.4, 2, theta)

val_theta = np.linspace(0, 0.601, 1_000)
val_y = poder(val_theta)

plt.figure(figsize=(8, 5))
plt.plot(val_theta, val_y, color="blue")
plt.xlim(0, 0.601)
plt.xlabel(r'$\theta$')
plt.ylabel(r'$1 - \pi(\theta)$')
plt.show()
```

### R
```{r}
#| label: fig-teste-hipotese-beta-r
#| fig-cap: 'Função erro tipo II $\beta=1-\pi(\theta)=1-(1-\theta)^2$'
library(ggplot2)

# P(X > 0,8) = P(X >= 1), para X com distribuição Binomial(2, theta)
poder <- function(theta) {
  pbinom(2 * 0.4, size = 2, prob = theta, lower.tail = FALSE)
}
...
```{julia}
#| echo : false
#| label: fig-teste-hipotese-poder-normal
#| fig-cap: 'Função poder $\alpha=\pi(\theta)$ sob normalidade'
...
a = plot(
    poder, xlims = (mu0 - 6 * sqrt(sigma2 / n), mu0 + 6 * sqrt(sigma2 / n)), ylims = (0, 1.001), label = "",
...
beta(mu0)
b = plot(
    beta, xlims = (mu0 - 6 * sqrt(sigma2 / n), mu0 + 6 * sqrt(sigma2 / n)), ylims = (-0.01, 1.001), label = "",
...
```{julia}
#| echo: false
#| label: fig-teste-hipotese-beta-normal
#| fig-cap: 'Função erro tipo II $\beta$ sob normalidade'
display(b)
```
...
2. Considere $n=100$, $\mu_{0}=1$, $\sigma^2=0.1$ e $\bar{x}=0.99$. Conclua o teste considerando o mesmo nível de significância $\alpha=5\%$.
	 ...
	 Os pontos de corte são $x_{a} = 0.93$ e $x_{b} = 1.069$. Como $0.93\leq 0.99 \leq 1.069$, concluímos que não há
...
$$
\begin{aligned}
&\frac{(n-1)s^2(\boldsymbol{X}_{n})}{\sigma^2_{0}} \sim \chi^2_{n-1}
\\ \Rightarrow& \alpha_{\max} = \sup_{\theta \in \Theta_{0}}\left\{ P_{\theta}(s^2(\boldsymbol{X}_{n}) < c_{1c}) +
P_{\theta}(s^2(\boldsymbol{X}_{n})> c_{2c}) \right\}
\end{aligned}
$$
