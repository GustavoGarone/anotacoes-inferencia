...
$$
P_\theta(T(\boldsymbol{X}_n)=t) = \begin{cases}
...
$$
P_\theta\left(\frac{1}{n}\sum^n_{i=1}X_i=\frac{k}{n}\right) = \begin{cases}
...
$$
P_\theta\left(\sum^n_{i=1}X_i=k\right) = \begin{cases}
...
\frac{f_\theta^{\boldsymbol{X}_n, T(\boldsymbol{X}_n)}(y_1,\dots,y_n,t)}
...
f_\theta^{\boldsymbol{X}_n}(y_1,\dots,y_n) \cdot
f_\theta^{T(\boldsymbol{X}_n)\lvert \boldsymbol{X}_n=(y_1,\dots,y_n)}(t)
$$

Note que
$$
\begin{aligned}
&f_\theta^{T(\boldsymbol{X}_n)\lvert \boldsymbol{X}_n = (y_1,\dots,y_n} = \begin{cases}
1, & t = \sum^n_{i=1} y_1 \\
0, & \mathrm{c.c.}
\end{cases} \\
\Rightarrow&
...
Logo
$$
f_\theta^{T(\boldsymbol{X}_n)\lvert \boldsymbol{X}_n = (y_1,\dots,y_n)} =
\begin{cases}
\frac{\frac{1}{\sqrt{2\pi\cdot1}^n} \cdot \mathrm{exp}\left\{-\frac{1}{2} \sum^n_i=1 (y_i - \theta)^2\right\}}
{\frac{1}{\sqrt{2\pi\cdot n}}\cdot \mathrm{exp}\left\{-\frac{1}{2n}(t-n\theta)^2\right\}}, & t = \sum_{i=1}^n y_i \\
0, & \mathrm{c.c.}
\end{cases}
$$
...
Note que
$$
-\frac{1}{2}\sum_{i=1}^n(y_i-\theta)^2=-\frac{1}{2}\left(\frac{t^2}{n}-2t\theta+n\theta^2\right)
$$
logo $f_\theta^{\boldsymbol{X}_n\lvert T(\boldsymbol{X}_n)=t}$ não depende de $\theta$ e $\sum_{i=1}^n X_i$ é suficiente para o modelo
estatístico.
...
0, & x \not \in (0,\infty)
\end{cases}\ \ \theta \in \Theta \\
P_\theta(X>2)&=\int^\infty_2 \theta \cdot \mathrm{e}^{-\theta x} dx \\
&\text{Se $A$ é enumerável e definimos} \\
...
$$
\begin{aligned}
f_\theta(x) &= f_\theta^A(x), \forall x \in \mathbb{R} \setminus A, \forall
\theta \in \Theta \\
\text{e} \\
...
Ou, de outra forma,
$$
P_\theta\left(f_\theta(x)\neq f_\theta^A(x)\right) = 0, \forall \theta \in
\Theta
$$
...
$$
f_\theta^{\boldsymbol{X}_n}(y_1,\dots,y_n) = h(y_1,\dots,y_n)\cdot
m\left(T(y_1,\dots,y_n),\theta\right), \forall \theta \in \Theta\; \mathrm{q.c.}
$$
...
$$
L_{\boldsymbol{X}_n}(\theta) =
f_\theta^{\boldsymbol{X}_n}\underbracket{(x_1,\dots,x_n)}_{\boldsymbol{X}_n}
$$

5. Alguns livros usam a função de verossimilhança no critério da fatoração
$$
L_{\boldsymbol{X}_n}(\theta) = h(\boldsymbol{X}_n) \cdot
m(T(\boldsymbol{X}_n),\theta)\; \mathrm{q.c.} \forall \theta \in \Theta
$$
...
$$
P_\theta(X_1=y_1,\dots,X_n=y_n\lvert T(\boldsymbol{X}_n)=t) =
h^*(y_1,\dots,y_n,t) \forall \theta \in \Theta
$$ {#eq-podemos}
Note também que,
$$
P_\theta(X_1=y_1,\dots,X_n=y_n\lvert T(\boldsymbol{X}_n)=t) =
...
&P_\theta(X_1=y_1,\dots,X_n=y_n, T(\boldsymbol{X}_n)=t) \\
&=
P_\theta(X_1=y_1,\dots,X_n=y_n)\cdot P_\theta(T(\boldsymbol{X}_n)=t\lvert
...
$$
P_\theta(T(\boldsymbol{X}_n)=t\lvert X_1=y_1,\dots,X_n=y_n) = \begin{cases}
...
$$
h^*(y_1,\dots,y_n, t) \cdot P_\theta(T(\boldsymbol{X}_n)=t) =
P_\theta(X_1=y_1,\dots,X_n=y_n\lvert \mathbb{1}(T(\boldsymbol{X}_n)=t)).
$$

Para $T(\boldsymbol{X}_n)=t$, temos que
$$
P_\theta(X_1=y_1,\dots,X_n=y_n) = h^*(y_1,\dots,y_n, T(\boldsymbol{y}_n)) \cdot
P_\theta(T(\boldsymbol{X}_n)=T(\boldsymbol{y}_n))
$$

#### $\Leftarrow$
Assuma que existam $h, m$ tais que
$$
P_\theta(X_1=y_1,\dots,X_n=y_n) = h(\boldsymbol{y}_n)
m(T(\boldsymbol{y}_n,\theta))
$$
Note que
$$
P_\theta(X_1=y_1,\dots,X_n=y_n\lvert T(\boldsymbol{X}_n) = t) = \begin{cases}
...
Observe que
$$
P_\theta(T(\boldsymbol{X}_n) = t) = \sum_{(y_1,\dots,y_n) :
T(\boldsymbol{y})_n=t} P_\theta(X_1=y_1,\dots,X_n=y_n)
$$
Por suposição
$$
\begin{aligned}
P_\theta(T(\boldsymbol{X}_n) =t) &= \sum_{\boldsymbol{y}_n :
T(\boldsymbol{y}_n)=t} h(\boldsymbol{y}_n) \cdot m (T(\boldsymbol{y}_n), \theta)
\\
...
$$
\begin{aligned}
f_\theta^{\boldsymbol{X}_n}(y_1,\dots,y_n) &= \begin{cases}
\prod^n_{i=1}\left\{\theta^{y_i}\cdot(1-\theta)^{1-y_i}\right\}, & y_i \in \{0,1\}, \forall i = 1,\dots,n \\
0, & \mathrm{c.c.}
\end{cases} \\
\Rightarrow
f_\theta^{\boldsymbol{X}_n}(y_1,\dots,y_n) &= \theta^{\sum_{i=1}^n y_i} \cdot
(1-\theta)^{n-\sum_{i=1}^n y_i} \cdot \prod^n_{i=1}\mathbb{1}_{\{0,1\}} (y_1)
\end{aligned}
$$

Tome $h(y_1,\dots,y_n) = \prod^n_{i=1}\mathbb{1}_{\{0,1\}}(y_1)$ e $m(T(y_1,\dots,y_n),\theta) = \theta^{\sum_{i=1}^n y_i} \cdot (1-\theta)^{n-\sum_{i=1}^n y_i}$
em que $T(y_1,\dots,y_n) = \sum^n_{i=1}y_i$. Temos que
$$
f_\theta^{\boldsymbol{X}_n}(y_1,\dots,y_n) = h(y_1,\dots,y_n)
m(T(y_1,\dots,y_n),\theta),\forall \theta \in \Theta
$$
...
$$
\Rightarrow f_\theta^{\boldsymbol{X}_n}(\boldsymbol{y}_n) = \begin{cases}
\frac{1}{\beta(a,b)^n}
...
$$
f_\theta^{\boldsymbol{X}_n}(\boldsymbol{y}_n) = m(T(\boldsymbol{y}_n),\theta)\
\mathrm{q.c.}\ \forall \theta \in \Theta.
$$
...
$$
\begin{aligned}
f_\theta^{\boldsymbol{X}_n} (y_1,\dots,y_n) &\stackrel{\mathrm{a.a.}}{=}
\prod_{i=1}^n f_\theta(y_i), \forall \boldsymbol{y}_n \in \mathbb{R}^n \\
&= \prod_{i=1}^n \left\{
...
$$
f_\theta^{\boldsymbol{X}_n} = \frac{1}{(2\pi\sigma^2)^{\frac{n}{2}}} \cdot
\mathrm{exp} \left\{
-\frac{1}{\sigma^2}\left(
...
$$
f_\theta^{\boldsymbol{X}_n}(\boldsymbol{y}_n) \cdot
m(T(\boldsymbol{X}_n),\theta)\ \mathrm{q.c.}\ \forall \theta \in \Theta.
$$
...
0, & \mathrm{c.c.}
\end{cases} = \frac{1}{\theta}\mathbb{1}(x)_{(0,\theta]}
$$
...
$$
é uma estatística suficiente se $s(\cdot)$ for bijetora (só precisa ser injetora)

### Prova

Como $T(\boldsymbol{X}_n)$ é suficiente para o modelo temos, pelo critério da fatoração, que
$$
f_\theta^{\boldsymbol{X}_n}(\boldsymbol{y}_n) = h(\boldsymbol{y}_n) \cdot
m(T(\boldsymbol{y}_n),\theta)\ \mathrm{q.c.}\ \forall \theta \in \Theta
$$
...
$$
f_\theta^{\boldsymbol{X}_n} (\boldsymbol{y}_n)= h(\boldsymbol{y}_n) \cdot
m(s^{-1}(G(\boldsymbol{X}_n)), \theta)\ \mathrm{q.c.}\ \forall \theta \in \Theta
$$
...
$$
f_\theta^{\boldsymbol{X}_n} (\boldsymbol{y}_n)= h(\boldsymbol{y}_n) \cdot
m^*((G(\boldsymbol{X}_n)), \theta)\ \mathrm{q.c.}\ \forall \theta \in \Theta
$$

Logo, pelo [critério da fatoração](estatisticas-suficientes.qmd#sec-crit-fat), $G(\boldsymbol{X}_n)$ é suficiente para o
modelo estatístico.
...
### Corolário útil (Relação das suficientes com a FE)
Se $f_\theta$ pertencer à [família exponencial k-dimensional](familia-exponencial.qmd#sec-fek), então
...
Tome $h(\boldsymbol{y}_n) = \prod_{i=1}^n \mathbb{1}_{\mathfrak{X}}(y_i)\cdot \mathrm{e}^{\sum_{i=1}^n S(y_i)}$,
$m(t,\theta) = \mathrm{exp}\left\{c_1(\theta)t_1+\dots+c_k(\theta)t_k +nd(\theta)\right\}$, em que $t=(t_1,\dots,t_k)$ e
$$
\begin{aligned}
t_1 &= \sum_{i=1}^n T_1(y_i) \\
\vdots \\
t_k & = \sum_{i=1}^n T_k(y_i)
\end{aligned}
$$

Pelo critério da fatoração,
$$
T(\boldsymbol{X}_n) = \left(\sum^n_{i=1}
T_1(X_i),\dots,\sum^n_{i=1}T_k(X_i)\right)
$$

Isso é útil para encontrar [estatísticas completas](estatisticas-completas.qmd) e suficientes pelo
[Teorema das FEs](estatisticas-completas.qmd#sec-teo-tres-completa).

## Estatísticas Suficientes Minimais (SM) {#sec-sm}
...
Isso trata de *razões entre [funções de verossimilhança](funcao-verossimilhanca.qmd)*.
...
$$
f_\theta^{\boldsymbol{X}_n}(\boldsymbol{y}_n) = h'(\boldsymbol{y}_n) \cdot
m'(U(\boldsymbol{y}_n),\theta)\ \mathrm{q.c.}\ \forall \theta \in \Theta_0
$$
...
$$
\begin{aligned}
f_{0.1}^{\boldsymbol{X}_n} (\boldsymbol{y}_n) &= 0.1^{\sum y_i} \cdot
0.9^{n-\sum y_i} \\
f_{0.5}^{\boldsymbol{X}_n} (\boldsymbol{y}_n) &= 0.5^{\sum y_i} \cdot
0.5^{n-\sum y_i} \\
...
### Teorema (2 das estatísticas SM) {#sec-teo-dois}
...
$$
T(\boldsymbol{X}_n) =
\frac{f_{\theta_1}^{\boldsymbol{X}_n}(\boldsymbol{X}_n)}{f_{\theta_0}^{\boldsymbol{X}_n}(\boldsymbol{X}_n)}
$$
é SM para o modelo reduzido.

$$
T(\boldsymbol{X}_n) =
\frac{\frac{1}{(2\pi)^{\frac{n}{2}}}\mathrm{exp}\left\{-\frac{1}{2} (\sum X_i^2
-2\sum X_i + n)\right\}}
...
$$
T(\boldsymbol{X}_n) =
\left(\frac{f_{\theta_1}^{\boldsymbol{X}_n}(\boldsymbol{X}_n)}{f_{\theta_0}^{\boldsymbol{X}_n}(\boldsymbol{X}_n)},
...
"$\theta$", não podemos usar o teorema anterior. Precisamos mostrar que, para qualquer estatística suficiente para
...
$$
\begin{aligned}
f_\theta^{\boldsymbol{X}_n}(\boldsymbol{y}_n) &= \frac{1}{\theta^n} \prod
\mathbb{1}_{(0,\theta]}(y_i) \\
&= \frac{1}{\theta^n} \mathbb{1}_{(0,\infty)} (\min\{y_1,\dots,y_n\}) \cdot \mathbb{1}_{(0,\theta]}(\max\{y_1,\dots,y_n\})
\end{aligned}
$$

Fixando $\boldsymbol{y}_n$ com $y_i>0$ para todo $i$ e escrevendo
$y_{(n)}=\max\{y_1,\dots,y_n\}$, podemos visualizar essa densidade como função de $\theta$:

```{.tikz width=80%}
%%| format: tex
%%| filename: densidade-uniforme-funcao-theta
%%| fig-cap: Densidade amostral do modelo uniforme como função do parâmetro
...
\begin{tikzpicture}[x=1.3cm,y=1.15cm]
  ...
  % Eixos
  \draw[->,thick] (0,0) -- (5.4,0) node[right] {$\theta$};
  \draw[->,thick] (0,-0.25) -- (0,2.9)
    node[above] {$f_\theta^{\mathbf{X}_n}(\mathbf{y}_n)$};

  % Região em que a densidade é nula
  \draw[blue!65!black,very thick] (0.05,0) -- (\m,0);
  \draw[blue!65!black,fill=white,thick] (\m,0) circle (2pt);

  % Salto em y_(n) e ramo theta^{-n}
  \draw[densely dashed] (\m,0) -- (\m,\h);
  \draw[densely dashed] (0,\h) -- (\m,\h);
  \fill[blue!65!black] (\m,\h) circle (2pt);
  \draw[blue!65!black,very thick,->]
    (\m,\h) .. controls (2.65,1.15) and (3.8,0.45) .. (5.15,0.2);

  % Marcas e rótulos
  \node[below] at (\m,0) {$y_{(n)}$};
  \node[left] at (0,\h) {$y_{(n)}^{-n}$};
  \node[blue!65!black] at (1,0.38) {$f_\theta=0$};
  \node[blue!65!black] at (3.65,1.25) {$f_\theta=\theta^{-n}$};
  \node[align=center] at (\m,-0.7)
...
$$
X_{(n)} = \inf\left\{\theta \in (0,\infty) : f^{\boldsymbol{X}_n}_\theta
(\boldsymbol{X}_n) > 0\right\}
$$
...
$$
f_\theta{\boldsymbol{y}_n}^{\boldsymbol{X}_n} = h'(\boldsymbol{y}_n) \cdot
m'(U(\boldsymbol{y}_n), \theta),\ \mathrm{q.c.}\ \forall \theta >0
$$

Logo,
$$
X_{(n)} = \inf\left\{\theta \in (0,\infty) : h'(\boldsymbol{X}_n) \cdot
m'(U(\boldsymbol{X}_n),\theta) > 0\right\} \mathrm{q.c.}
$$

Como $h'(\boldsymbol{y}_n)> 0\ \mathrm{q.c.}$ temos que

$$
X_{(n)} = \inf\left\{\theta \in (0,\infty) : m'(U(\boldsymbol{X}_n),\theta)>0
\right\} \mathrm{q.c.}
$$
...
#### Exemplo (Normal Curvada) {#sec-norm-curv}
...
Tome $\Theta_0 = \{1,2,3\}$. Pelo [Teorema 1](estatisticas-suficientes.qmd#sec-teo-um), temos que:
$$
T'(\boldsymbol{X}_n) =
\left(\frac{f_{\theta_1}^{\boldsymbol{X}_n}(\boldsymbol{X}_n)}{f_{\theta_0}^{\boldsymbol{X}_n}(\boldsymbol{X}_n)},
\frac{f_{\theta_2}^{\boldsymbol{X}_n}(\boldsymbol{X}_n)}{f_{\theta_0}^{\boldsymbol{X}_n}(\boldsymbol{X}_n)}\right)
...
$$
\begin{aligned}
f_{\theta_0}^{\boldsymbol{X}_n} &=
\frac{\mathrm{exp}\left\{-\frac{1}{2}\sum(X_i-1)^2\right\}}{(\sqrt{2\pi})^n}
= \frac{\mathrm{exp}\left\{-\frac{1}{2}(\sum X_i^2 - 2 \sum X_i
+n)\right\}}{(\sqrt{2\pi})^n} \\
f_{\theta_1}^{\boldsymbol{X}_n} &= \frac{\mathrm{exp}\left\{-\frac{1}{2\cdot
4}(\sum X_i^2 - 2\cdot 2 \cdot \sum X_i +4n)\right\}}{(\sqrt{2\pi\cdot4})^n} \\
f_{\theta_2}^{\boldsymbol{X}_n} &= \frac{\mathrm{exp}\left\{-\frac{1}{2\cdot
9}(\sum X_i^2 - 2\cdot 3 \cdot \sum X_i +9n)\right\}}{(\sqrt{2\pi\cdot9})^n}.
\end{aligned}
$$
Portanto,
$$
...
\end{cases}\\
\Rightarrow&
t_2 = \frac{\ln(9^{\frac{n}{2}}t'_2) -
\frac{12}{9}(4^{\frac{n}{2}}t'_1)}{\left(\frac{4}{9} - \frac{18}{36}\right)} \\
\Rightarrow&
\begin{cases}
t_1 = \frac{\frac{3}{4}\left(\ln(9^{\frac{n}{2}}t_2'\right) -
\frac{12}{9}\ln(4^{\frac{n}{2}}t'_1)}{\left(\frac{4}{9}-\frac{18}{36}\right)} -
2\ln(4^{\frac{n}{2}}t'_1)\\
t_2 = \frac{ln(9^{\frac{n}{2}}t'_2) -
\frac{12}{9}(4^{\frac{n}{2}}t'_1)}{\left(\frac{4}{9} - \frac{18}{36}\right)}
\end{cases}
\end{aligned}
...
Logo $T(\boldsymbol{X}_n)=(\sum X_i,\sum X_i^2)$  é SM apara o modelo reduzido. Como é suficiente para o modelo completo,
temos pelo [Teorema 2](estatisticas-suficientes.qmd#sec-teo-dois) que também é SM para o modelo completo.
