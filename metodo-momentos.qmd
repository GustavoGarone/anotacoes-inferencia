...
= \frac{\alpha}{\beta} \\
\Rightarrow E_\theta(X^2) &= \frac{1}{\Gamma(\alpha) \beta^2} \Gamma(\alpha +2) = \frac{(\alpha+1) \Gamma(\alpha+1)}{\Gamma(\alpha) \beta}
...
\Rightarrow &\left\{\begin{array}{ll}
\alpha = \bar{X}\beta \\
\underbracket{\beta^2\left(\frac{1}{n}\sum X_i^2 - \bar{X}^2\right) - \beta\bar{X}}_{S^2} = 0
\end{array}\right. \\ \\
