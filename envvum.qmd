...
$$
\begin{aligned}
E_\theta\left(\sum{1}{\sigma^2} \sum (X_i-\bar{X})^2\right) &\stackrel{\chi^2_{n-1}}{=} n-1 \\
...
$$
em que $\bar{X} = \frac{1}{n} \sum X_i$ e $S^2_{n-1} = \frac{1}{n-1} \sum (X_i - bar{X})^2$, é suficiente completa para
...
Intuitivamente, podemos esperar que $E_\theta(\sqrt{S^2_{n-1}})= c \sigma + b$. Se or, então bastaria tomar
$$
S'(\boldsymbol{X}_n) = \frac{\sqrt{S^2_{n-1}}-b}{c}
$$
para obtermos um estimador não viciado. Sob normalidade, $\bar{X}$ e $S^2_{n-1}$ são independentes. Logo, $\bar{X}$ e
$S'_{n-1}$ também são independentes. Portanto,
$$
E_\theta(S'(\boldsymbol{X}_n)\lvert T''(\boldsymbol{X}_n)) \stackrel{\mathrm{Ind.}}{=} E_\theta(S'(\boldsymbol{X}_n) \lvert S^2_{n-1}) 
...
Observe que
$$
W(\sigma^2) = \frac{(n-1)S^2_{n-1}}{n} \sim \chi^2_{n-1}.
$$
Logo, para $n>1$
$$
\begin{aligned}
E_\theta(\sqrt{S^2_{n-1}}) &= E_\theta\left(\frac{\sqrt{n-1}}{\sqrt{n-1}}\frac{\sqrt{\sigma^2}}{\sqrt{\sigma^2}} \sqrt{S^2_{n-1}}\right) \\
&= E_\theta\left(\frac{\sqrt{\sigma^2}}{\sqrt{n-1}}\sqrt{\frac{(n-1)S^2_{n-1}}{\sigma^2}}\right) \\
...
Dessa forma,
$$
E_\theta\left(\sqrt{S^2_{n-1}}\right) = \frac{\sigma}{\sqrt{n-1}} \frac{\sqrt{2}}{\Gamma\left(\frac{n-1}{2}\right)} \Gamma\left(\frac{n}{2}\right),
$$
tomando
$$
S'(\boldsymbol{X}_n) = \frac{\sqrt{(n-1) S^2_{n-1}}}{\sqrt{2}} \frac{\Gamma\left(\frac{n-1}{2}\right)}{\Gamma\left(\frac{n}{2}\right)},
$$
é, como $S'(\boldsymbol{X}_n) = c\sqrt{S^2_{n-1}(\boldsymbol{X}_n)}$ é função 1:1 de $S^2_{n-1}$, o ENVVUM para $g(\theta) = \sigma$.
...
Portanto, subistituindo $t$ por $T(\boldsymbol{X}_n)$, temos que
$$ 
