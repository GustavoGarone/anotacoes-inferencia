...
$$
\mathcal{L}_{\boldsymbol{x}_n}(\theta) \stackrel{\mathrm{iid}}{=} \prod^n_{i=1} f_\theta(x_i) = \prod_{i=1}^n\left\{\frac{1}{\theta} \mathbb{1}_{(0,\theta]}(x_i)\right\}
= \frac{1}{\theta^n} \prod_{i=1}^n \mathbb{1}_{(0,\theta]}(x_i)
$$

Note que
$$
\prod_{i=1}^n \mathbb{1}_{(0,\theta]}(x_i) = 1 \iff \mathbb{1}_{[0,\infty)}(\min\boldsymbol{x}_n) \mathbb{1}_{(-\infty,\theta]}(\max\boldsymbol{x}_n) = 1
$$
...
temos que $\theta = \max\boldsymbol{x}_n$  é o ponto que maximiza a função de verossimilhança, logo,
$$
...
$$
\ln\mathcal{L}_{\boldsymbol{x}_n}(\theta) = n \ln \theta + (\theta-1) \sum_{i=1}^n \ln x_i
$$
...
&\Rightarrow \frac{n}{\theta} + \sum_{i=1}^n \ln x_i = 0 \\
&\iff \hat{\theta} = - \frac{n}{\sum_{i=1}^n \ln x_i}\ \text{é um ponto crítico.}
\end{aligned}
$$
...
Logo,
$$
\hat{\theta}_{\mathrm{MV}}(\boldsymbol{X}_n) = - \frac{n}{\sum_{i=1}^n \ln x_i}
$$
é o EMV.
...
Note que a informação de Fisher total sob o [modelo normal
anterior](emv2.qmd#sec-exnorm) é
$$
\begin{aligned}
...
Posteriormente, [encontraremos um método](metodo-nr.qmd) para encontrar essa
solução.
...
Em alguns casos, ([-@eq-formageral]) não tem solução fechada e utilizaremos
um método numérico para encontrar as soluções.
:::
