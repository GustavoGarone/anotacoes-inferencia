...
def wald(dist, theta_0):
    xbar = medias_amostrais(dist)
    estatistica = np.full(MC, np.inf)
    positivos = xbar > 0
    estatistica[positivos] = (
        n * (xbar[positivos] - theta_0) ** 2 / xbar[positivos]
    )
    return estatistica
...
1. Observe que, quanto menor for o valor-$p$, há mais evidências de que $\mathcal{H}_0$ é falsa.
...
que, quanto maior o valor observado de $T_{\boldsymbol{H}_0}$, mais evidências contra $\mathcal{H}_0$. Além disso, sob $\mathcal{H}_0$,
...
$$
U_n(\boldsymbol{X}_n, \theta) = -n + \frac{1}{\theta_0} \sum_{i=1}^n X_i
$$
e a [Informação de Fisher Total](infofisher.qmd) por
$$
I_n(\theta_0) = -E_\theta\left(-\sum_{i=1}^n \frac{X_i}{\theta_0} \right) = \frac{n}{\theta_0},
$$
portanto,
$$
\begin{aligned}
T'''_{\mathcal{H}_0} (\boldsymbol{X}_n) &= \left(\frac{\sum_{i=1}^n X_i}{\theta_0} - n \right)^2 \cdot \frac{\theta_0}{n} \\
...
```{python}
n = 10
p1 = chi2.sf(razao(poisson(mu=1), 1), df=1)
p2 = chi2.sf(wald(poisson(mu=1), 1), df=1)
p3 = chi2.sf(escore(poisson(mu=1), 1), df=1)

def graficos_valores_p(p1, p2, p3):
    # Compara os valores-p das três estatísticas.
    fig, eixos = plt.subplots(2, 2, figsize=(9, 7))
    titulos = (
        "Valores-P da Razão de Ver.",
        "Valores-P de Wald",
        "Valores-P de Escore",
    )
    for eixo, valores, titulo in zip(
        (eixos[0, 0], eixos[0, 1], eixos[1, 0]),
        (p1, p2, p3),
        titulos,
    ):
        eixo.hist(valores, bins="auto", range=(0, 1), density=True)
        eixo.set_title(titulo)
        eixo.set(xlabel="valor-p", ylabel="Densidade")
    eixos[1, 1].axis("off")
    fig.tight_layout()
    jl.display(plt.gcf())

graficos_valores_p(p1, p2, p3)
```
...
```{r}
n <- 10
p1 <- pchisq(razao(1, 1), df = 1, lower.tail = FALSE)
p2 <- pchisq(wald(1, 1), df = 1, lower.tail = FALSE)
p3 <- pchisq(escore(1, 1), df = 1, lower.tail = FALSE)

graficos_valores_p <- function(p1, p2, p3) {
  # Organiza os três histogramas em uma grade 2 x 2.
  configuracao_anterior <- par(mfrow = c(2, 2))
  on.exit(par(configuracao_anterior))

  hist(p1, probability = TRUE, main = "Valores-P da Razão de Ver.", xlab = "valor-p")
  hist(p2, probability = TRUE, main = "Valores-P de Wald", xlab = "valor-p")
  hist(p3, probability = TRUE, main = "Valores-P de Escore", xlab = "valor-p")
  plot.new()
}
...
#| output: asis
print(
    rf"$\hat P(\mathrm{{val.-}}p_1 < 0.05) = {np.mean(p1 < 0.05):.4f}, "
    rf"\hat P(\mathrm{{val.-}}p_2 < 0.05) = {np.mean(p2 < 0.05):.4f}, "
    rf"\hat P(\mathrm{{val.-}}p_3 < 0.05) = {np.mean(p3 < 0.05):.4f}$"
)
```
...
cat(sprintf(
  paste0(
    "$\\hat P(\\mathrm{val.-}p_1 < 0.05) = %.4f, ",
    "\\hat P(\\mathrm{val.-}p_2 < 0.05) = %.4f, ",
    "\\hat P(\\mathrm{val.-}p_3 < 0.05) = %.4f$"
  ),
  mean(p1 < 0.05), mean(p2 < 0.05), mean(p3 < 0.05)
))
```
:::
...
```{python}
n = 100
p1 = chi2.sf(razao(poisson(mu=0.01), 0.01), df=1)
p2 = chi2.sf(wald(poisson(mu=0.01), 0.01), df=1)
p3 = chi2.sf(escore(poisson(mu=0.01), 0.01), df=1)
...
```{r}
n <- 100
p1 <- pchisq(razao(0.01, 0.01), df = 1, lower.tail = FALSE)
p2 <- pchisq(wald(0.01, 0.01), df = 1, lower.tail = FALSE)
p3 <- pchisq(escore(0.01, 0.01), df = 1, lower.tail = FALSE)
...
```{python}
n = 10
p1 = chi2.sf(razao(poisson(mu=2), 1), df=1)
p2 = chi2.sf(wald(poisson(mu=2), 1), df=1)
p3 = chi2.sf(escore(poisson(mu=2), 1), df=1)
...
```{r}
n <- 10
p1 <- pchisq(razao(2, 1), df = 1, lower.tail = FALSE)
p2 <- pchisq(wald(2, 1), df = 1, lower.tail = FALSE)
p3 <- pchisq(escore(2, 1), df = 1, lower.tail = FALSE)
