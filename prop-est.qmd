...
Com isso,
$$
\sqrt{n}(\hat{\theta}_{\mathrm{MM}}(\boldsymbol{X}_n) - \theta) \rightarrow{\mathcal{D}}{\rightarrow} N(0, V_\theta)
$$
em que
$$
V_\theta = \left(\frac{E_\theta[(X^k - E_\theta(X^k)^2]}{\frac{\partial E_\theta(X^k)}{\partial \theta}}\right)^2 = \frac{\mathrm{Var}_\theta (X^k)}{h(\theta)^2}
$$
...
\rightarrow{\mathcal{D}}{\rightarrow} N\left(0,g'(\theta)^2 \frac{\mathrm{Var}_\theta (X^k)}{h(\theta)^2}\right)
$$

:::{.callout-note title="Notação"}
$$
\begin{cases}
\hat{\theta}_{\mathrm{MM}}(\boldsymbol{X}_n) &\stackrel{a}{\approx} N\left(\theta,\frac{\mathrm{Var}_\theta (X^k)}{h(\theta)^2}\right) \\
g(\hat{\theta}_{\mathrm{MM}}(\boldsymbol{X}_n)) &\stackrel{a}{\approx} N\left(g(\theta), g'(\theta)^2 \frac{\mathrm{Var}_\theta (X^k)}{h(\theta)^2}\right) \\
\end{cases}
$$
:::
...
```{julia}
#| echo: true
using Distributions, Random, Plots, StatsPlots, StatsBase
...
theta00 = MV(x)# rand(Uniform(0,100))
dsim = Beta(theta00, 1)
...
normMV = Normal(theta00, sqrt(VMV(theta00)))
normMM = Normal(theta00, sqrt(VMM(theta00)))

histmv = histogram(MVs, normalize=:pdf, label="", color=:blue,
                   title="Histograma EMV")
plot!(normMV, label="Aprox. Norm.")
vline!([MV(x)], label="EMV da amostra", color=:orange)
histmm = histogram(MMs, normalize=:pdf, label="", color=:red,
                   title="Histograma EMM")
plot!(normMM, label="Aprox. Norm.")
vline!([MM(x)], label="EMM da amostra", color=:yellow)
p = plot(histmv, histmm)

display(p)
println("Chute para Theta (Theta00): $theta00")
println("Theta real (normalmente desconhecido): $theta0")
println("Viés MM: $(mean(MMs .- theta00))")
println("EQM MM: $(mean((MMs .- theta00).^2))")
println("Viés MV: $(mean(MVs .- theta00))")
println("EQM MV: $(mean((MVs .- theta00).^2))")
println("EMV da amostra: $(MV(x))")
println("EMM da amostra: $(MM(x))")
println("P(MM > 2.5) = $(cdf(normMM, 2.5))")
println("P(MV > 2.5) = $(cdf(normMV, 2.5))")
println("Real MM > 2.5 = $(mean(MMs .> 2.5))")
println("Real MV > 2.5 = $(mean(MVs .> 2.5))")
```
