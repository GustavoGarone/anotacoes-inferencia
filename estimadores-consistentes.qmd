...
$$
Apesar de ser uma forma comum de exposição para pessoas não treinadas em
estatística, essa notação não está bem
...
::: {.callout-tip}
Disso, temos que, se $T(\pmb{X}_n)$ for [assintoticamente eficiente](estimadores-eficientes.qmd#sec-efassin), então será *fracamente* consistente.
:::
...
$$
\begin{aligned}
E_\theta(T_k(\pmb{X}_n)) &= E_\theta\left(\frac{1}{n-k} \sum_{i=1}^n (X_i - \bar{X})^2\right) \\
&= \frac{\sigma^2}{n-k}E_\theta\left(\sum_{i=1}^n \frac{(X_i - \bar{X})^2}{\sigma^2}\right) \\
...
$$
\begin{aligned}
\mathrm{Var}_\theta(T_k(\pmb{X}_n)) &= \mathrm{Var}_\theta\left(\frac{1}{n-k} \sum_{i=1}^n (X_i - \bar{X})^2\right) \\
&= \frac{\sigma^4}{(n-k)^2}\mathrm{Var}_\theta\left(\sum_{i=1}^n \frac{(X_i - \bar{X})^2}{\sigma^2}\right) \\
