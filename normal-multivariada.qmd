...
$$
f^{\boldsymbol{X}}(\boldsymbol{x}) = \frac{1}{(2\pi)^{\frac{d}{2}} \lvert \Sigma
\rvert^{\frac{1}{2}}}
...
$$
\begin{aligned}
(\boldsymbol{x} - \boldsymbol{\mu})^{T}\Sigma^{-1}(\boldsymbol{x} -
\boldsymbol{\mu}) &=
\left(\begin{array}{c}
x_1 - \mu_1 \\
\vdots \\
...
$$
\begin{aligned}
f^{\boldsymbol{X}}(\boldsymbol{x}) &= \frac{1}{(2\pi)^{\frac{d}{2}} \left(\prod_{i=1}^d
\sigma_{ii}\right)^{\frac{1}{2}}}
\mathrm{Exp}\left\{-\frac{1}{2}\sum_{i=1}^d \frac{(x_i - \mu_i)^2}{(\sigma_{ii})}\right\} \\
&= \prod_{i=1}^d \left\{
...
$$
\begin{aligned}
M_{\boldsymbol{Y}}(\boldsymbol{t} &=
E(\mathrm{e}^{\boldsymbol{Y}^T\boldsymbol{t}}) \\
...
Como $\Sigma$ é uma matriz quadrada simétrica e positiva definida, todos seus autovalores são estritamente positivos. Logo,
$$
\Sigma = \Gamma \Lambda \Gamma^T
$$
em que $\Gamma$ é a matriz ortogonal cujas colunas são os autovetores de $\Sigma$ e $\Lambda$ é a matriz diagonal de autovalores de $\Sigma$.
...
**Verificação numérica.** Considere a matriz simétrica
$$
\Sigma = \begin{bmatrix}
2 & 1 & 0 \\
1 & 2 & 0 \\
...
\end{bmatrix}.
$$
O código abaixo calcula sua decomposição espectral e verifica numericamente as identidades anteriores.
...
decomposicao = eigen(Σ)
Λ = Diagonal(decomposicao.values)
Γ = decomposicao.vectors

Σ_reconstruida = Γ * Λ * transpose(Γ)
Σ_raiz_inversa = Γ * Diagonal(diag(Λ) .^ (-1 / 2)) * transpose(Γ)
I_recuperada = Σ_raiz_inversa * Σ * transpose(Σ_raiz_inversa)

println("Autovalores de Σ: ", round.(diag(Λ), digits=6))
println("Erro na decomposição Σ = ΓΛΓᵀ: ", norm(Σ - Σ_reconstruida))
println("Erro na identidade Σ⁻¹ᐟ²ΣΣ⁻¹ᐟ² = I: ", norm(I - I_recuperada))
println("det(Σ): ", round(det(Σ), digits=6))
println("Produto dos autovalores: ", round(prod(diag(Λ)), digits=6))
println("tr(Σ): ", round(tr(Σ), digits=6))
println("Soma dos autovalores: ", round(sum(diag(Λ)), digits=6))
println("Σ⁻¹ᐟ²:")
display(round.(Σ_raiz_inversa, digits=4))
```
...
# eigh() explora a simetria de Sigma e devolve os autovalores em ordem crescente.
autovalores, Gamma = np.linalg.eigh(Sigma)
Lambda = np.diag(autovalores)

Sigma_reconstruida = Gamma @ Lambda @ Gamma.T
Sigma_raiz_inversa = Gamma @ np.diag(autovalores ** (-1 / 2)) @ Gamma.T
I_recuperada = Sigma_raiz_inversa @ Sigma @ Sigma_raiz_inversa.T

print("Autovalores de Sigma:", np.round(autovalores, 6))
print("Erro na decomposição Sigma = Gamma Lambda Gammaᵀ:",
      np.linalg.norm(Sigma - Sigma_reconstruida))
print("Erro na identidade Sigma⁻¹/² Sigma Sigma⁻¹/² = I:",
      np.linalg.norm(np.eye(3) - I_recuperada))
print("det(Sigma):", round(np.linalg.det(Sigma), 6))
print("Produto dos autovalores:", round(np.prod(autovalores), 6))
print("tr(Sigma):", round(np.trace(Sigma), 6))
print("Soma dos autovalores:", round(np.sum(autovalores), 6))
print("Sigma⁻¹/²:")
print(np.round(Sigma_raiz_inversa, 4))
```
...
decomposicao <- eigen(Sigma, symmetric = TRUE)
# eigen() usa ordem decrescente; reordenamos para acompanhar Julia e Python.
ordem <- order(decomposicao$values)
autovalores <- decomposicao$values[ordem]
Gamma <- decomposicao$vectors[, ordem]
Lambda <- diag(autovalores)

Sigma_reconstruida <- Gamma %*% Lambda %*% t(Gamma)
Sigma_raiz_inversa <- Gamma %*% diag(autovalores^(-1 / 2)) %*% t(Gamma)
I_recuperada <- Sigma_raiz_inversa %*% Sigma %*% t(Sigma_raiz_inversa)

cat("Autovalores de Sigma:", round(autovalores, 6), "\n")
cat("Erro na decomposição Sigma = Gamma Lambda Gammaᵀ:",
    norm(Sigma - Sigma_reconstruida, type = "F"), "\n")
cat("Erro na identidade Sigma⁻¹/² Sigma Sigma⁻¹/² = I:",
    norm(diag(3) - I_recuperada, type = "F"), "\n")
cat("det(Sigma):", round(det(Sigma), 6), "\n")
cat("Produto dos autovalores:", round(prod(autovalores), 6), "\n")
cat("tr(Sigma):", round(sum(diag(Sigma)), 6), "\n")
cat("Soma dos autovalores:", round(sum(autovalores), 6), "\n")
cat("Sigma⁻¹/²:\n")
print(round(Sigma_raiz_inversa, 4))
```
:::

Os autovalores obtidos são $1$, $3$ e $4$; como todos são positivos, $\Sigma$ é positiva definida. As normas das diferenças
nas duas primeiras verificações são próximas de zero, havendo apenas erro de arredondamento numérico. Além disso,
$$
\lvert\Sigma\rvert = 12 = 1\cdot 3\cdot 4
\qquad\text{e}\qquad
\mathrm{tr}\{\Sigma\} = 8 = 1+3+4.
$$
Por fim, o cálculo de $\Sigma^{-1/2}\Sigma\Sigma^{-1/2}$ recupera a matriz identidade, como exigido na padronização
...
-\frac{1}{2} \left(\begin{array}{c} X_1 - \mu_1 \\ X_2 - \mu_2
\end{array}\right)^T
\left(\begin{array}{cc} \sigma_{11} & -\sigma_{12} \\ -\sigma_{12} & \sigma_{22} \end{array}\right)
\left(\begin{array}{c} X_1 - \mu_1 \\ X_2 - \mu_2 \end{array}\right) \frac{1}{\sigma_{11}\sigma_{22} - \sigma_{12}^2}
\right\}}_{*} \\ \\
*&= [(x_1 - \mu_1)\sigma_{22} - (x_2 - \mu_2)\sigma_{12} -
(x_1-\mu_1)\sigma_{12}+(x_2 - \mu_2)\sigma_{11}]
...
A.\ \frac{\sigma_{11}\sigma_{22} - \sigma_{12}^2}{\sigma_{22}} &= \sigma_{11} -
\frac{\sigma_{12}^2}{\sigma_{22}} \\
&= \sigma_{11} - \sigma_{11}\frac{\sigma_{12}^2}{\sigma_{11}\sigma_{22}} \\
&= \sigma_{11} - \sigma_{11} \rho^2  \\
&= \sigma_{11}(1-\rho^2)\\ \\
B.\ \frac{\sigma_{11} \sigma_{22} - \sigma_{12}^2}{\sigma_{11}} &=
\sigma_{22}(1-\rho^2) \\ \\
C.\ \frac{\sigma_{11}\sigma_{22}}{\sigma_{12}} - \sigma_{12} &= \sigma_{12}
\frac{\sigma_{11}\sigma_{22}}{\sigma_{12}^2} - \sigma_{12} \\
...
-2\rho^2\frac{(x_1 - \mu_1)(x_2 - \mu_2)}{\sigma_{12}(1-\rho^2)} + \frac{(x_2 -
\mu_2)^2}{\sigma_{22}(1-\rho^2)}
\end{aligned}
$$
...
-2\rho\frac{(x_1 - \mu_1)(x_2 - \mu_2)}{\sqrt{\sigma_{11}\sigma_{22}}} +
\frac{(x_2 - \mu_2)^2}{\sigma_{22}} \right)
\right\}
\end{aligned}
$$
...
: \Sigma\ \text{é positiva definida (p.d.)}\right\}\subseteq \mathbb{R}^{d +
\frac{d(d+1)}{2}}
\end{aligned}
$$
...
$$
\begin{aligned}
\mathcal{L}_{\boldsymbol{x}^*}(\theta) &=  \prod_{i=1}^n f_\theta^{\boldsymbol{X}}(\boldsymbol{x}_i) =
\prod_{i=1}^n \left\{
\frac{1}{(2\pi)^{\frac{d}{2}}\lvert\Sigma\rvert^{\frac{1}{2}}} \mathrm{Exp}\left\{
-\frac{1}{2}(\boldsymbol{x}_i - \boldsymbol{\mu})^T \Sigma^{-1}(\boldsymbol{x}_i
- \boldsymbol{\mu})
\right\}
\right\} \\
&=  \frac{1}{(2\pi)^{\frac{nd}{2}}\lvert\Sigma\rvert^{\frac{1}{2}}} \mathrm{Exp}\left\{
-\frac{1}{2}\underbracket{\sum_{i=1}^n(\boldsymbol{x}_i - \boldsymbol{\mu})^T
\Sigma^{-1}(\boldsymbol{x}_i - \boldsymbol{\mu})}_{\dagger}
\right\}
\end{aligned}
$$

Defina $\bar{\boldsymbol{x}} = \frac{1}{n} \sum_{i=1}^n \boldsymbol{x}_i$, então
$$
\begin{aligned}
\dagger &= \sum_{i=1}^n (\boldsymbol{x}_i - \bar{\boldsymbol{x}} + \bar{\boldsymbol{x}} - \boldsymbol{\mu})^T
\Sigma^{-1} (\boldsymbol{x}_i - \bar{\boldsymbol{x}} + \bar{\boldsymbol{x}} - \boldsymbol{\mu}) \\
&= \sum_{i=1}^n ((\boldsymbol{x}_i - \bar{\boldsymbol{x}}) - (\boldsymbol{\mu} - \bar{\boldsymbol{x}}))^T
\Sigma^{-1} ((\boldsymbol{x}_i - \bar{\boldsymbol{x}}) - (\boldsymbol{\mu} - \bar{\boldsymbol{x}})) \\
&= \sum_{i=1}^n \left\{
(\boldsymbol{x} - \bar{\boldsymbol{x}})^T \Sigma^{-1} (\boldsymbol{x}_i -
\bar{\boldsymbol{x}})
-\underbracket{(\boldsymbol{\mu} - \bar{\boldsymbol{x}})^T \Sigma^{-1}
(\boldsymbol{x}_i - \bar{\boldsymbol{x}})}_{A} \right .\\
&\left.-\underbracket{(\boldsymbol{x}_i - \bar{\boldsymbol{x}})^T \Sigma^{-1} (\boldsymbol{\mu} - \bar{\boldsymbol{x}})}_{B}
+(\boldsymbol{\mu} - \bar{\boldsymbol{x}})^T \Sigma^{-1} (\boldsymbol{\mu} -
\bar{\boldsymbol{x}})
\right\}
\end{aligned}
$$

Como $n\bar{\boldsymbol{x}} = \sum_{i=1}^n \boldsymbol{x}_i$, temos que

$$
\begin{cases}
A = \sum_{i=1}^n (\boldsymbol{\mu} - \bar{\boldsymbol{x}})^T \Sigma^{-1}
(\boldsymbol{x}_i - \bar{\boldsymbol{x}}) =
(\boldsymbol{\mu} - \bar{\boldsymbol{x}})^T \Sigma^{-1} (\sum_{i=1}^n \boldsymbol{x}_i
- \bar{\boldsymbol{x}}) = 0 \\
B = \sum_{i=1}^n (\boldsymbol{x}_i - \bar{\boldsymbol{x}})^T \Sigma^{-1}
(\boldsymbol{\mu} - \bar{\boldsymbol{x}}) =
(\sum_{i=1}^n \boldsymbol{x}_i - \bar{\boldsymbol{x}})^T \Sigma^{-1} (\boldsymbol{\mu}
- \bar{\boldsymbol{x}}) = 0
\end{cases}
$$
...
\begin{aligned}
\dagger &=
\sum_{i=1}^n (\boldsymbol{x} - \bar{\boldsymbol{x}})^T \Sigma^{-1} (\boldsymbol{x}_i - \bar{\boldsymbol{x}})
+n(\boldsymbol{\mu} - \bar{\boldsymbol{x}})^T \Sigma^{-1} (\boldsymbol{\mu} -
\bar{\boldsymbol{x}}) \\
&=  \sum_{i=1}^n \mathrm{tr}\{(\boldsymbol{x} - \bar{\boldsymbol{x}})^T \Sigma^{-1} (\boldsymbol{x}_i - \bar{\boldsymbol{x}})\}
+n(\boldsymbol{\mu} - \bar{\boldsymbol{x}})^T \Sigma^{-1} (\boldsymbol{\mu} -
\bar{\boldsymbol{x}}) \\
&\stackrel{\text{Prop. tr}}{=}  \sum_{i=1}^n \mathrm{tr}\{\Sigma^{-1}(\boldsymbol{x} - \bar{\boldsymbol{x}})(\boldsymbol{x}_i - \bar{\boldsymbol{x}})^T\}
+n(\boldsymbol{\mu} - \bar{\boldsymbol{x}})^T \Sigma^{-1} (\boldsymbol{\mu} -
\bar{\boldsymbol{x}}) \\
\end{aligned}
$$

Note que $\sum_{i=1}^n \mathrm{tr}\{A \boldsymbol{y}_i\} = \mathrm{tr}\left\{A \sum_{i=1}^n \boldsymbol{y}_i\right\}$, usando essa propriedade,
$$
\begin{aligned}
...
\end{aligned}
$$
em que $S^2(\boldsymbol{x}_n^*) = \frac{1}{n} \sum_{i=1}^n (\boldsymbol{x}_i - \bar{\boldsymbol{x}})(\boldsymbol{x}_i - \bar{\boldsymbol{x}})^T$.
...
$$
-\frac{nd}{2} \ln(2\pi) - \frac{n}{2} \ln\lvert\Sigma\rvert - \frac{n}{2}
\mathrm{tr}\{\Sigma^{-1}S^2(\boldsymbol{x}_n^*)\} =
c - \frac{n}{2}\left( \ln\lvert\Sigma\rvert +
\mathrm{tr}\{\Sigma^{-1}S^2(\boldsymbol{x}_n^*)\}\right)
$$
...
\Rightarrow &\sum_{i=1}^d \lambda_i - \sum_{i=1}^d \ln \lambda_i \geq d \\
\iff& \sum_{i=1}^d \lambda_i - \ln\left(\prod_{i=1}^d \lambda_i\right) \geq d \\
...
5. Para uma amostra observada $\boldsymbol{x}_n^*$, apresente as estimativas para $g(\theta)$ dos itens anteriores:
...
(\boldsymbol{X}_i - \bar{\boldsymbol{X}})(\boldsymbol{X}_i -
\bar{\boldsymbol{X}})^T \\
&= \frac{1}{n} \sum_{i=1}^n \boldsymbol{X}_i\boldsymbol{X}_i^T - \bar{\boldsymbol{X}}\bar{\boldsymbol{X}}^T
\end{aligned}
$$
...
\sum_{i=1}^n x_{2i}
\end{pmatrix}\\
\hat{\Sigma}_{\mathrm{MV}}(\boldsymbol{x}^*_n) &= S^2(\boldsymbol{x}_n^*) = \frac{1}{n} \sum_{i=1}^n \boldsymbol{x}_i\boldsymbol{x}_i^T - \bar{\boldsymbol{x}}\bar{\boldsymbol{x}}^T\\
...
Pela propriedade de invariância:

1. Como $E_\theta(\boldsymbol{X}) = \boldsymbol{\mu}$,
$$
g(\hat{\theta}_{\mathrm{MV}}(\boldsymbol{X}_n^*))
= \hat{\mu}_{\mathrm{MV}}(\boldsymbol{X}_n^*)
= \bar{\boldsymbol{X}}.
$$

2. Como $\mathrm{Var}_\theta(\boldsymbol{X}) = \Sigma$,
$$
g(\hat{\theta}_{\mathrm{MV}}(\boldsymbol{X}_n^*))
= \hat{\Sigma}_{\mathrm{MV}}(\boldsymbol{X}_n^*)
= S^2(\boldsymbol{X}_n^*).
$$

3. Como $g(\theta) = \boldsymbol{\mu}^T\Sigma^{-1}\boldsymbol{\mu}$,
$$
\begin{aligned}
g(\hat{\theta}_{\mathrm{MV}}(\boldsymbol{X}_n^*))
&= \hat{\mu}_{\mathrm{MV}}(\boldsymbol{X}_n^*)^T
\hat{\Sigma}_{\mathrm{MV}}(\boldsymbol{X}_n^*)^{-1}
\hat{\mu}_{\mathrm{MV}}(\boldsymbol{X}_n^*) \\
&= \bar{\boldsymbol{X}}^T S^2(\boldsymbol{X}_n^*)^{-1}\bar{\boldsymbol{X}}.
\end{aligned}
$$

4. $P_\theta(X_1 \geq 4X_2) = P_\theta(X_1 - 4X_2 \geq 0)$. Note que
$$
X_1 - 4X_2 = B \boldsymbol{X}
$$
em que $B = (1, -4)$. Então,
$$
X_1 - 4X_2 \sim N(B\boldsymbol{\mu}, B\Sigma B^T)
$$
Desenvolvendo,
$$
\begin{aligned}
B\boldsymbol{\mu} &= \mu_1 - 4\mu_2 \\
B\Sigma B^T &= (1,-4) \begin{bmatrix}\sigma_{11} & \sigma_{12} \\ \sigma_{21} &
\sigma_{22} \end{bmatrix} \begin{pmatrix}1 \\ -4 \end{pmatrix} \\
&\stackrel{\sigma_{12} = \sigma_{21}}{=} \sigma_{11} + 16\sigma_{22} - 8 \sigma_{12}
\end{aligned}
$$
...
$$
\begin{aligned}
g(\theta) &= P_\theta(X_1 - 4X_2 \geq 0) \\
&= P\left(Z \geq -\frac{\mu_1 - 4\mu_2}
{\sqrt{\sigma_{11} + 16\sigma_{22} - 8\sigma_{12}}}\right) \\
&= \Phi\left(\frac{\mu_1 - 4\mu_2}
{\sqrt{\sigma_{11} + 16\sigma_{22} - 8\sigma_{12}}}\right).
\end{aligned}
$$

Escrevendo $S^2(\boldsymbol{X}_n^*) = [S_{ij}(\boldsymbol{X}_n^*)]_{i,j=1}^2$, pela propriedade de invariância,
$$
g(\hat{\theta}_{\mathrm{MV}}(\boldsymbol{X}_n^*))
= \Phi\left(\frac{\bar{X}_1 - 4\bar{X}_2}
{\sqrt{S_{11}(\boldsymbol{X}_n^*) + 16S_{22}(\boldsymbol{X}_n^*) - 8S_{12}(\boldsymbol{X}_n^*)}}\right).
$$

Finalmente, escrevendo $S^2(\boldsymbol{x}_n^*) = [s_{ij}]_{i,j=1}^2$, as estimativas pedidas no item 5 são,
respectivamente,
$$
\begin{aligned}
g_1(\hat{\theta}_{\mathrm{MV}}(\boldsymbol{x}_n^*))
&= \bar{\boldsymbol{x}}, \\
g_2(\hat{\theta}_{\mathrm{MV}}(\boldsymbol{x}_n^*))
&= S^2(\boldsymbol{x}_n^*), \\
g_3(\hat{\theta}_{\mathrm{MV}}(\boldsymbol{x}_n^*))
&= \bar{\boldsymbol{x}}^T S^2(\boldsymbol{x}_n^*)^{-1}\bar{\boldsymbol{x}}, \\
g_4(\hat{\theta}_{\mathrm{MV}}(\boldsymbol{x}_n^*))
&= \Phi\left(\frac{\bar{x}_1 - 4\bar{x}_2}
{\sqrt{s_{11} + 16s_{22} - 8s_{12}}}\right).
\end{aligned}
$$
...
ou, de outra forma,
$$
I_n(\theta)^{\frac{1}{2}}(\hat{\theta}_{\mathrm{MV}}(\boldsymbol{X}_n) - \theta)
\stackrel{\mathcal{D}}{\rightarrow} N_p(\boldsymbol{0}, \boldsymbol{I}).
$$
Além disso, pelo [Teorema de Slutsky](slutsky.qmd),
$$
I_n(\hat{\theta}_{\mathrm{MV}}(\boldsymbol{X}_n))^{\frac{1}{2}}(\hat{\theta}_{\mathrm{MV}}(\boldsymbol{X}_n)
- \theta)
\stackrel{\mathcal{D}}{\rightarrow} N_p(\boldsymbol{0}, \boldsymbol{I})
$$
...
$$
E_\theta(X^k) = \frac{1}{n} \sum_{i=1}^n X_i^k, k \in \{k_1, \dots, k_n\}
$$
Seja
$$
h(\theta) = \begin{bmatrix}\frac{\partial E_\theta(X^{k_1})}{\partial \theta^T}
\\ \vdots \\ \frac{\partial E_\theta(X^{k_p})}{\partial \theta^T} \end{bmatrix}
$$
...
Encontre o EMV, EMM e suas distribuições assintóticas para $\theta$.

##### Resposta
Note que $B(\alpha, \beta) = \frac{\Gamma(\alpha) \Gamma(\beta)}{\Gamma(\alpha + \beta)}$
$$
\begin{aligned}
\mathcal{L}_{\boldsymbol{x}_n}(\theta) &= \frac{1}{B(\alpha, \beta)^n} \left(\prod_{i=1}^n x_i\right)^{\alpha-1} \left(\prod_{i=1}^n(1-x_i)\right)^{\beta - 1} \\
...
A estimativa de MV pode ser obtida numericamente via [algoritmo de
Newton-Raphson](metodo-nr.qmd). Neste caso, o método
...
-n \psi_1(\alpha) + n \psi_1(\alpha + \beta) +\sum_{i=1}^n \ln x_i \\
-n \psi_1(\beta) + n \psi_1(\alpha + \beta) +\sum_{i=1}^n \ln(1-x_i)
\end{bmatrix}
\end{aligned}
$$
...
&\cdot
\begin{bmatrix}
\psi_2(\beta) - \psi_2(\alpha + \beta) & \psi_2(\alpha+\beta) \\
\psi_2(\alpha+\beta) & \psi_2(\alpha) - \psi_2(\alpha+\beta)
\end{bmatrix}.
\end{aligned}
$$
...
```{julia}
#| echo: true
using Distributions, Random, SpecialFunctions, LinearAlgebra
Random.seed!(1)
function main()
    n = 10
    alpha0 = 1.2
    ...
    while erro > erromax && i <= iteracoesMax
        thetahat0 = thetahat
        thetahat = thetahat .+ inv(In(thetahat)) * U(thetahat)
...
def main():
    ...
    alpha0 = 1.2
    beta0 = 7
    x = rng.beta(alpha0, beta0, size=n)

    def U(theta):
        # Vetor escore da distribuição beta.
        alpha, beta = theta
        U1 = -n * digamma(alpha) + n * digamma(alpha + beta) + np.log(x).sum()
        U2 = -n * digamma(beta) + n * digamma(alpha + beta) + np.log1p(-x).sum()
        return np.array([U1, U2])

    def I1(theta):
        # Informação de Fisher por observação.
        alpha, beta = theta
        I11 = polygamma(1, alpha) - polygamma(1, alpha + beta)
        I22 = polygamma(1, beta) - polygamma(1, alpha + beta)
        I12 = -polygamma(1, alpha + beta)
        return np.array([[I11, I12], [I12, I22]])

    thetahat = np.array([0.1, 0.1])
    erro_maximo = 0.0005
    erro = np.inf
    i = 1
    iteracoes_maximas = 1_000

    while erro > erro_maximo and i <= iteracoes_maximas:
        thetahat_anterior = thetahat.copy()
        # Resolver o sistema linear evita calcular explicitamente a inversa.
        thetahat = thetahat + np.linalg.solve(n * I1(thetahat), U(thetahat))
        erro = np.max(np.abs(thetahat - thetahat_anterior))
        print(f"Erro na iteração {i}: {erro}")
        i += 1
    ...
    print("Theta final:", thetahat)
    print("Total de iterações:", i - 1)
    return thetahat
...
main <- function() {
  ...
  alpha0 <- 1.2
  beta0 <- 7
  x <- rbeta(n, shape1 = alpha0, shape2 = beta0)

  U <- function(theta) {
    alpha <- theta[1]
    beta <- theta[2]
    U1 <- -n * digamma(alpha) + n * digamma(alpha + beta) + sum(log(x))
    U2 <- -n * digamma(beta) + n * digamma(alpha + beta) + sum(log1p(-x))
    c(U1, U2)
  }
  ...
  I1 <- function(theta) {
    alpha <- theta[1]
    beta <- theta[2]
    I11 <- trigamma(alpha) - trigamma(alpha + beta)
    I22 <- trigamma(beta) - trigamma(alpha + beta)
    I12 <- -trigamma(alpha + beta)
  ...
  while (erro > erro_maximo && i <= iteracoes_maximas) {
    thetahat_anterior <- thetahat
    # solve(A, b) resolve A x = b sem calcular explicitamente A^(-1).
    thetahat <- thetahat + solve(n * I1(thetahat), U(thetahat))
    erro <- max(abs(thetahat - thetahat_anterior))
    cat(sprintf("Erro na iteração %d: %g\n", i, erro))
    i <- i + 1
  }

  if (erro > erro_maximo) {
    stop(sprintf("O método não convergiu em %d iterações.", iteracoes_maximas))
  }

  cat("Theta final:", thetahat, "\n")
