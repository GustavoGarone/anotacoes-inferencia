...
<!-- TODO: tudo isso e bonitificar tabela -->
<!--
```{julia}
#|eval: false
using Random, Distributions, DataFrames, LinearAlgebra
Random.seed!(1)
...
a = 100
b = -3
X1 = mu1 .+ rand(Normal(0, sqrt(sigma11)), n)
X2 = a .+ b .* X1 .+ rand(Normal(0, sqrt(sigma11)), n)
X1 = round.(X1, digits=2)
X2 = round.(X2, digits=2)
df = DataFrame(Salário= X1, Insatisfação=X2)
BD = [X1 X2]
# muMV = mean.(BD)
# soma1 = transpose(BD) .* BD
# variancia = var(BD)
# print(variancia)
# display(mean.(eachcol(df)))
# display(var.(eachcol(df)))
# display(df)
```
-->
