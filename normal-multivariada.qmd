...
G = eigvecs(b)
raiz_inversa_direta = b^(-1 / 2)
raiz_inversa_decomp = G * Diagonal(diag(L) .^ (-1 / 2)) * transpose(G)
println("Gama: $G")
println("Lambda: $L")
println("Matriz raiz quadrada inversa (direto): $raiz_inversa_direta")
println("Matriz raiz quadrada inversa (decomposição): $raiz_inversa_decomp")
println("Norma da diferença: $(norm(raiz_inversa_direta - raiz_inversa_decomp))")
...
B\Sigma B^T &= (-4,1) \begin{bmatrix}\sigma_{11} & \sigma_{12} \\ \sigma_{21} &
\sigma_{22} \end{bmatrix} \begin{pmatrix}-4 \\ 1 \end{pmatrix} \\
