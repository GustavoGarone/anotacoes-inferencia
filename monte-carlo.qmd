...
Como estudamos no último capítulo sobre viés, vimos no [exemplo](eqm-e-vies.qmd#sec-ex-viesvar)
...
function simula_montecarlo(theta0, M, n, corrigido = false)
    ...
    for i in 1:M
        rand!(dist, amostra) # Funções terminadas em ! são mutadoras
    ...
    vies_teorico = corrigido ? 0.0 : -theta0/n # O ? é o operador ternário, como um if.
    println("\n" * "="^30)
...
para aplicações diversas, para além da área de estatística e ciência de dados.
```{python}
