...
-\frac{1}{4} \sum_{i=1}^n (x_i - \theta)^{2} \right\} \\ &=
\left(\frac{1}{\sqrt{4\pi}}\right)^n \mathrm{Exp}\left\{
-\frac{1}{4} \sum_{i=1}^n x_i^2 - 2\theta\sum_{i=1}^n x_i + n\theta^2 \right\}.
\end{aligned}
$$
Pelo [Critério de Fatoração de Fisher](estatisticas-suficientes.qmd#sec-crit-fat), $T(\boldsymbol{X}_n) = \sum_{i=1}^n x_i$ é uma estatística suficiente.
Note que
$$
\begin{aligned}
\mathrm{R}_{\theta',\theta''}(T(\boldsymbol{X}_n)) &= \frac{\left(\frac{1}{\sqrt{4\pi}}\right)^n \mathrm{Exp}\left\{
-\frac{1}{4} \sum_{i=1}^n x_i^2 - 2\theta'\sum_{i=1}^n x_i + n\theta'^2 \right\}}{\left(\frac{1}{\sqrt{4\pi}}\right)^n \mathrm{Exp}\left\{
-\frac{1}{4} \sum_{i=1}^n x_i^2 - 2\theta''\sum_{i=1}^n x_i + n\theta''^2 \right\}} \\
...
em que $c$ satisfaz
$$
\pi_{\delta^*}(1) = P_{1}(\sum_{i=1}^n X_i \geq c) = \alpha.
$$

Com $n = 5$, temos que, quando $\theta = 1$, $\sum_{i=1}^n X_i \sim N(1 \cdot 5, 5 \cdot 2)$. Assim,
$$
\begin{aligned}
P_{1}(\sum_{i=1}^n X_i \geq c) &= P_1\left(\frac{\sum_{i=1}^n x_i - 5}{\sqrt{10}} \geq \frac{c - 5}{\sqrt{10}}\right) = 0.1 \\
\Rightarrow \frac{c - 5}{\sqrt{10}} &= 1.28 \\
\Rightarrow c &= 1.28 \cdot \sqrt{10} + 5 = 9.064 \\
\Rightarrow  \delta^*(\boldsymbol{x}_n) &= \begin{cases}
0,&\ \sum_{i=1}^n x_i < 9.064 \\
1,&\ \sum_{i=1}^n x_i \geq 9.064
\end{cases}
\end{aligned}
$$
Portanto, se $\sum_{i=1}^n x_i < 9.064$, dizemos que *não há evidências* para rejeitarmos $\mathcal{H}_0$ a $10\%$ de significância
estatística. Se $\sum_{i=1}^n x_i \geq 9.064$, dizemos que *há evidências* para rejeitarmos $\mathcal{H}_0$ a $10\%$ de significância
estatística.
...
$$
\begin{aligned}
\mathcal{L}_{\boldsymbol{x}_n}(\theta) &= \theta^n \cdot \mathrm{e}^{-\theta \sum_{i=1}^n x_i}.
\end{aligned}
$$
...
$$
\begin{aligned}
\mathrm{R}_{\theta',\theta''}(T(\boldsymbol{X}_n)) &= \frac{\theta'^n \cdot \mathrm{e}^{-\theta' \sum_{i=1}^n x_i}}{\theta''^n \cdot \mathrm{e}^{-\theta'' \sum_{i=1}^n x_i}} \\
...
Tome $T'(\boldsymbol{x}_n) = - \sum_{i=1}^n x_i$, logo
$$
...
em que $c$ satisfaz
$$
\pi_{\delta^*}(2) = P_{2}(\sum_{i=1}^n X_i \leq -c) = \alpha.
$$
Para $n=10$, sob $\theta = 2, \sum_{i=1}^n X_i \sim \mathrm{Gama}(10,2)$. Computacionalmente, o quantil $0.05$ dessa distribuição
é $2.7127$. Logo, $-c = 2.7127 \Rightarrow c = -2.7127$. Dessa forma, se $\sum_{i=1}^n x_i > 2.7127$, dizemos que *não há evidências* para rejeitarmos $\mathcal{H}_0$ a $5\%$ de significância
estatística. Se $\sum_{i=1}^n x_i \leq 2.7127$, dizemos que *há evidências* para rejeitarmos $\mathcal{H}_0$ a $5\%$ de significância estatística.
...
em que $c$ satisfaz
$$
\pi_{\delta^*}(1) = P_{1}(\sum_{i=1}^n X_i \leq c) = \alpha.
$$
...
# Valores do teste e da simulação
n = 10
α = 0.05
MC = 10_000

# O ponto crítico para a soma, c_soma = -c, é calculado uma única vez,
# na borda de Θ_0.
dist_soma_h0 = Gamma(n, 1 / θ_0)
c_soma = quantile(dist_soma_h0, α)

function proporcao_rejeicoes(θ, c_soma)
    d = Exponential(1 / θ)
    rejeicoes = zeros(MC)
    for i in 1:MC
        x = rand(d, n)
        rejeicoes[i] = sum(x) ≤ c_soma
    end
    return mean(rejeicoes)
end

erro_tipo_i = proporcao_rejeicoes(θ_0, c_soma)
poder = proporcao_rejeicoes(θ_1, c_soma)

println("Ponto crítico para a soma: $c_soma")
println("Testes rejeitados na borda de H_0: $(erro_tipo_i * 100)%")
println("Poder do teste em θ = $θ_1: $(poder * 100)%")
```
...
# Seja X a.a. de X ~ Exp(theta)
# H_0: theta <= 2; H_1: theta > 2
# delta(x) = 1 se, e somente se, sum(x) <= c_soma
...
# O ponto crítico para a soma, c_soma = -c, é calculado uma única vez,
# na borda de Theta_0.
c_soma = gamma.ppf(alpha, a=n, scale=1 / theta_0)

def proporcao_rejeicoes(theta, c_soma):
    amostras = rng.exponential(scale=1 / theta, size=(MC, n))
    somas = np.sum(amostras, axis=1)
    return np.mean(somas <= c_soma)

erro_tipo_i = proporcao_rejeicoes(theta_0, c_soma)
poder = proporcao_rejeicoes(theta_1, c_soma)

print(f"Ponto crítico para a soma: {c_soma}")
print(f"Testes rejeitados na borda de H_0: {erro_tipo_i * 100}%")
print(f"Poder do teste em theta = {theta_1}: {poder * 100}%")
```
...
# O ponto crítico para a soma, c_soma = -c, é calculado uma única vez,
# na borda de Theta_0.
c_soma <- qgamma(alpha, shape = n, scale = 1 / theta_0)

proporcao_rejeicoes <- function(theta, c_soma) {
  amostras <- matrix(rexp(MC * n, rate = theta), nrow = MC)
...
cat("Ponto crítico para a soma:", c_soma, "\n")
cat("Testes rejeitados na borda de H_0:", erro_tipo_i * 100, "%\n")
cat("Poder do teste em theta =", theta_1, ":", poder * 100, "%\n")
```
:::
