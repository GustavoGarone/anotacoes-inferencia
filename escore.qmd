...
$$
U : \mathfrak{X} \times \Theta \rightarrow \mathbb{R}^P, \Theta \subseteq
\mathbb{R}^P
$$
...
$$
U_n(\boldsymbol{x}_n,\theta) = \frac{\partial \ln
L_{\boldsymbol{x}_n}(\theta)}{\partial \theta} = \left(
\begin{array}{c}
...
\begin{aligned}
U_n(\boldsymbol{x}_n, \theta) = \frac{\partial \ln
L_{\boldsymbol{x}_n}(\theta)}{\partial \theta}
&= \frac{\partial \ln }{\partial \theta} \prod_{i=1}^n f_\theta(x_i) \\
&= \frac{\partial }{\partial \theta} \sum_{i=1}^n \ln f_\theta(x_i) \\
&= \sum_{i=1}^n \frac{\partial \ln f_\theta(x_i)}{\partial \theta} \\
...
Note que $U_n(\boldsymbol{X}_n,\theta) = \sum_{i=1}^n U(X_i,\theta)$. Por $C_4(a)$, podemos calcular a esperança:
$$
\begin{aligned}
E_\theta(U_n(\boldsymbol{X}_n,\theta)) &= \sum_{i=1}^n E_\theta(U(X_i,\theta)) \\
...
= \int_{\mathfrak{X}} \frac{\partial}{\partial \theta} f_\theta(x) dx,\ \forall
\theta \in \Theta
$$
