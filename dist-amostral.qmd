...
3. Encontre as seguintes [quantidades de interesse](quantidade-de-interesse.qmd) em função de $\theta$
	1. $g(\theta)=E_\theta(X)$
...
2. Duas resoluções possíveis
    ...
    $$
    P_\theta(X_1=k_1,X_2=k_2,X_3=k_3)
    = \prod_{i=1}^3 P_\theta(X_i=k_i).
    $$

    $$
    \begin{array}{c|c}
    \boldsymbol{x} & P_\theta(\boldsymbol{X}=\boldsymbol{x}) \\
    \hline
    (0,0,0) & (1-\theta)^3 \\
    (0,0,1) & (1-\theta)^2\theta \\
    (0,1,0) & (1-\theta)^2\theta \\
  ...
  2. Enunciando a função
     ...
     $$\begin{aligned}
       ...
       &P_\theta(X_{1}=k_1,X_{2}=k_{2},X_{3}= k_{3})\stackrel{\text{i.i.d}}{=}\prod^{3}_{i=1}\{\theta^{k_{i}}
...
amostra.
Como esse vetor é uma a.a. (ou seja, variáveis aleatórias independentes e
identicamente distribuídas), temos que
$$
...
$\theta=(\mu, \sigma^{2}) \in \Theta=\mathbb{R}\times \mathbb{R}_{+}$. Considere $\boldsymbol{x}=(x_{1},\dots,x_{n})$
a amostra observada.
...
&= \frac{1}{(2 \pi \sigma^{2})^{\frac{n}{2}}}\cdot \mathrm{exp}\{- \frac{1}{2 \sigma^{2}} \sum\limits^{n}_{i=1}(x_{i}-\mu)^{2}  \}
\end{aligned}
$$
