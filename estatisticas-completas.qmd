...
&\stackrel{\mathrm{ind.}}{=} \prod^n_{i=1} P_\theta(X_1\leq t) \stackrel{\mathrm{id}}{=} \prod P_\theta(X\leqt) \\
...
em que
$$
P_\theta(X\leq t) = \left\{\begin{array}
0, & t \leq 0 \\
...
Pelo Teorema Fundamental do Cálculo: $\frac{d}{dt} \int^t_a f(x)dx = f(t) - f(a)$, temos que
...
$$
E_\theta(h(T(\boldsymbol{X}_n)) = E_\theta(X_i) = \theta - \theta = 0, \forall \theta \in \Theta.
$$
...
## Teorema 3 (da relação com a família exponencial)
...
Observe também que
$$
\mathcal{C} =\left \{(c_1(\theta),c_2(\theta)) : \theta \in \Theta\} = \{\left(-\frac{1}{2\sigma^2}, \frac{\mu}{\sigma^2}\right)\right\}
$$
