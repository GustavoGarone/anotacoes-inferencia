...
Mostre que $T(\boldsymbol{X}_n) = \sum X_i$ é uma estatística completa para o modelo.
...
E_\theta(\sum X_i^2) &\stackrel{\mathrm{iid}}{=} n E_\theta(X^2) = n(\theta^2+
\theta^2) = 2n\theta^2 \\
...
$$
E_\theta\left(\frac{(\sum X_i)^2}{n^2 + n}\right) = \theta^2, \forall \theta \in
\Theta.
$$

Note ainda que
$$
E_\theta\left(\frac{\sum X_i^2}{2n}\right) = \theta^2, \forall \theta \in
\Theta.
$$
...
$$
T(\boldsymbol{X}_n) = \left(\sum T_1(\boldsymbol{X}_n),\dots,\sum
