...
$$
\left\{\begin{array}{ll}
I_1(\theta) = - E_\theta\left(\frac{\partial}{\partial \theta^T} U(X,\theta)^T\right) \\
I_n(\theta) = - E_\theta\left(\frac{\partial}{\partial \theta^T} U_n(\boldsymbol{X}_n,\theta)^T\right)
\end{array}\right.\ \forall \theta \in \Theta
$$
...
\Rightarrow \ln L_\boldsymbol{X_n}(\theta) &= \sum \ln \binom{3}{x_i} + \sum x_i \ln \theta + \sum (3-x_i)\ln(1-\theta) \\
