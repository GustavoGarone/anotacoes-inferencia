...
A informação de Fisher **total** é a variância da função escore da
amostra:
$$
...
Sob as [condições](escore.qmd#sec-condicoes) $C_1:C_4$, temos que
...
Sabemos que $U(X,\theta) = \frac{\partial}{\partial\theta}\ln f_\theta(x)$. Além disso,
$$
...
$$
\begin{aligned}
I_n(\theta) &= E_\theta\left[\left( \sum_{i=1}^n U(X_i,\theta)\right)^2\right]\\
&= E_\theta(\sum_i\sum_j U(X_i,\theta)U(X_j,\theta)) \\
&= E_\theta\left(\sum_{i=1}^n U(X_i,\theta)^2 + \sum_{i\neq j} U(X_i,\theta) U(X_j,\theta)\right) \\
&= \sum_{i=1}^n E_\theta (U(X_i,\theta)^2) + \sum_{\substack{i,j=1\\i\neq j}}^n E_\theta(U(X_i,\theta)U(X_j,\theta)), \forall \theta \in \Theta.
\end{aligned}
$$

Como $X_1,\dots,X_n$ são i.i.d, temos que $U(X_1,\theta),\dots,U(X_n,\theta)$ também são i.i.d. Logo,
$$
I_n(\theta) = \sum_{i=1}^n E_\theta(U(X_i,\theta)^2) + \sum_{\substack{i,j=1\\i\neq j}}^n E_\theta(\cancelto{0}{U(X_i,\theta)}) E_\theta(\cancelto{0}{U(X_j,\theta)})
$$
...
$$
\begin{aligned}
E_\theta\left(\frac{\partial}{\partial\theta}U(\boldsymbol{X}_n, \theta)\right) &= E_\theta\left(\frac{\partial}{\partial \theta} \sum_{i=1}^n U(X_i,\theta)\right) \\
&= E_\theta\left(\sum_{i=1}^n \frac{\partial}{\partial \theta} U(X_i,\theta)\right) \\
...
Encontre a função [escore](escore.qmd) da amostra e a informação de Fisher
total.
...
Note que
$$
L_{\boldsymbol{X}_n}(\theta) = \prod_{i=1}^n \left\{ \theta x_i^{\theta-1}\right\} = \theta^n \left(\prod_{i=1}^n x_i\right)^{\theta-1},\ \forall \theta \in \Theta.
$$
...
&= \frac{\partial}{\partial \theta} \left[n \ln \theta + (\theta-1) \sum_{i=1}^n \ln x_i\right] \\
&= \frac{n}{\theta} + \sum_{i=1}^n \ln x_i, \forall \theta \in \Theta.
\end{aligned}
$$

A informação de Fisher total é, pelo
[teorema](infofisher.qmd#sec-teo-infofisher),
$$
...
$$
\begin{aligned}
L_{\boldsymbol{X}_n}(\theta) &= \prod_{i=1}^n f_\theta(x_i) = \prod_{i=1}^n \left\{\binom{3}{x_i} \theta^{x_i} (1-\theta)^{3-x_i}\right\} \\
\Rightarrow \ln L_{\boldsymbol{X}_n}(\theta) &= \sum_{i=1}^n \ln \binom{3}{x_i} + \sum_{i=1}^n x_i \ln \theta + \sum_{i=1}^n (3-x_i)\ln(1-\theta) \\
\Rightarrow U_n(\boldsymbol{x}_n,\theta) &= \sum_{i=1}^n \frac{x_i}{\theta} - \sum_{i=1}^n \frac{3-x_i}{1-\theta},\ \forall \theta \in \Theta.
\end{aligned}
$$
...
&= -E_\theta\left( - \sum_{i=1}^n \frac{X_i}{\theta^2} + \sum_{i=1}^n \frac{3-X_i}{(1-\theta)^2}(-1)\right) \\
&= E_\theta\left(\sum_{i=1}^n \frac{X_i}{\theta^2} + \sum_{i=1}^n \frac{3-X_i}{(1-\theta)^2}\right) \\
&= \sum_{i=1}^n \frac{E_\theta(X_i)}{\theta^2} + \sum_{i=1}^n \frac{3 - E_\theta(X_i)}{(1-\theta)^2} \\
...
$$
\begin{aligned}
E_\theta(U_n(\boldsymbol{X}_n,\theta) &= E_\theta\left(\sum_{i=1}^n \frac{X_i}{\theta} - \sum_{i=1}^n \frac{3-X_i}{1-\theta}\right) \\
&= \sum_{i=1}^n \frac{E_\theta(X_i)}{\theta} - \sum_{i=1}^n \frac{3-E_\theta(X_i)}{1-\theta} \\
...
&= E_\theta\left(\left[\sum_{i=1}^n \left(\frac{X_i}{\theta} - \frac{(3-X_i)}{1-\theta}\right)\right]^2\right) \\
&= E_\theta\left(\sum_i \sum_j \left(\frac{X_i}{\theta} - \frac{(3-X_i)}{1-\theta}\right)\left(\frac{X_j}{\theta} - \frac{(3-X_j)}{1-\theta}\right)\right) \\
&= \sum_{i=1}^n E_\theta\left(\left(\frac{X_i}{\theta} - \frac{(3-X_i)}{1-\theta}\right)^2\right) \\
&\cdot\sum_{\substack{i,j=1\\i\neq j}}^n E_\theta\left(\cancelto{0}{\frac{X_i}{\theta} - \frac{(3-X_i)}{1-\theta}}\right)
...
as [condições $C_1:C_4$](escore.qmd#sec-condicoes). Considere $T(\boldsymbol{X}_n)$ um [estimador não-viciado](estimadores.qmd#sec-est-naovies)
...
A quantidade $\frac{1}{I_n(\theta)}$ é chamada de *Limite Inferior de Cramér-Rao (LICR)*
:::
...
Seja $\boldsymbol{X}_n$ a.a. de $X \sim f_\theta, \theta \in \Theta$, em que $f_\theta$ satisfaz as [condições $C_1,C_4$](escore.qmd#sec-condicoes).
