...
$L_{\boldsymbol{x}}(\hat\theta_{n}) \geq L_{\boldsymbol{x}}(\theta)\forall \theta \in \Theta$ é dito ser uma
[estimativa](estimadores.qmd#sec-estimativa) de máxima verossimilhança (MV) para $\theta$. Observe que $\hat\theta_{n}$ depende da
...
\end{array}
$$
Considere que a amostra observada é $\boldsymbol{x}=(0,0,1)$.
...
$$
L_{\boldsymbol{x}}(\theta)\stackrel{\text{iid}}{=}\prod^{n}_{i=1}f_{\theta}(x_{i})  =
...
Para $\boldsymbol{x}=(0,0,1)$,
$$
L_{\boldsymbol{x}}(\theta) = \theta^{2} \cdot \theta^{2} \cdot (1-\theta -\theta^{2})^{0} = \theta^{4}
\forall \theta \in \Theta
$$

Substituindo $\forall \theta \in \Theta$:
$$
\theta = \frac{1}{2}\Rightarrow L_{\boldsymbol{x}}\left(\frac{1}{2}\right)=\frac{1}{16} ~~~~ \theta =
\frac{1}{3}\Rightarrow L_{\boldsymbol{x}}\left(\frac{1}{3}\right)=\frac{1}{81}
$$
...
Assuma que $\boldsymbol{x} = (x_{1},\dots,x_{n})$ é a amostra observada.
...
$$
\begin{aligned}
L_{\boldsymbol{x}}(\theta)&\stackrel{\text{iid}}{=}\prod^{n}_{i=1}f_\theta(x_{i}) = \prod^{n}_{i=1}
...
$$
\begin{aligned}
\frac{\partial\ln(L_{\boldsymbol{x}})}{\partial \mu} &= \frac{1}{\sigma^{2}}\sum\limits^{n}_{i=1}(x_{i}-\mu) =0
\Rightarrow \hat \mu =\frac{1}{n} \sum\limits^{n}_{i=1}x_{i} \\
\frac{\partial \ln(L_{\boldsymbol{x}})}{\partial \sigma^{2}} &= - \frac{n}{2\sigma^{2}} + \frac{1}{2\sigma^{4}}
...
(Provados em cálculo), ou seja, são as estimativas
de máxima verossimilhança para $\mu, \sigma^{2}$ respectivamente, e
$\hat{\mu}(X_{1},\dots,X_{n})=\bar{X}, \hat{\sigma^{2}}(X_{1},\dots,X_{n})=\frac{1}{n}\sum\limits^{n}_{i=1}(X_{i}-\bar{X})^{2}$
...
$$
\widehat{g(\theta)} = \frac{\sqrt{\frac{1}{n}\sum\limits^{n}_{i=1}(X_{i}-\bar{X})^2}}{\bar{X}}
$$
...
Para qualquer outra distribuição, existe um resultado aproximado pelo [Teorema
do Limite Central](tlc.qmd).
:::
