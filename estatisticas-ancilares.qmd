...
Logo, $S_1(\boldsymbol{X}_n)$ é independente de $S_2(\boldsymbol{X}_n)$. Defina
$$
...
2. $\sum_{i=1}^n \frac{(X_i - \mu)^2}{\sigma^2} \sim \chi^2_{n})$ (por função geradora de momentos e transformação de VAs);

3. $\sum_{i=1}^n \frac{(X_i - \bar{X})^2}{\sigma^2} \sim \chi^2_{n-1})$ (por função geradora de momentos, álgebra linear e transformação de VAs);
...
Se $W\sim\chi^2_k$ e $Q\sim\chi^2_m$ são independentes, então
